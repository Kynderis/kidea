...
**Hiện hành — số đo backend thật và màn vận hành Web W5, 2026-09-19.** Backend nguồn `0fda8b7` đã kiểm mới bốn preset với 116 file nguồn; Web cuối `48d4fc3`. M1 đếm toàn bộ tồn đọng; tuổi là khoảng bảo thủ, chưa là tuổi commit chính xác. M2 mới bao phủ lỗi dispatcher/invariant; M4 gắn đúng epoch/workshop/version đã xử lý. Profile bổ sung chỉ dành cho DB mới; DB cũ không bị migration/reset và số đo chưa có được báo UNKNOWN.

**Kết quả mới:** Web 102 unit và 40 SSR–Chrome Mac PASS trên API giả; backend thật qua Chrome/HTTPS đạt 144 kiểm trình duyệt và 27 SQL, giữ đủ 132/22 kiểm cũ. Đã kiểm và xem ảnh ở 360/1280 px, chữ 200%. Mỗi preset backend đạt thêm 167 kiểm telemetry và 77 kiểm updates, sạch sanitizer. Dev/ASan/release: 50 CTest/856 assertion PASS; TSan vẫn giữ raw 7 FAIL/14 report SQLite–WAL đúng EX r2 đã duyệt, 43 ca cũ sạch. Không thêm ngoại lệ hoặc tuyên bố toàn backend TSan sạch. Mọi lượt FAIL được giữ nguyên; cách gọi test được điều chỉnh giữa các ca độc lập, không đổi giới hạn hoặc kỳ vọng.

Public REVISE mở review revision 10 **DRAFT**; SAVE trả CONTINUATION_SAVED; checkpoint pilot local `94231a9`, không remote; READ từ commit mới trả WAITING. Chỉ nextAction/checkpointRef của work thay đổi; owners, trạng thái, gates, blockers và lịch sử nghiệm thu T04 giữ nguyên. Core 293/recovery 9 được dùng lại với 48 hash khớp, không phải lượt chạy mới. Observer 29/15 vẫn là bằng chứng thành phần nguồn giả. Ledger 137 và expected giữ nguyên byte; số 11 covered/88 partial/38 NOT_RUN được dùng lại, chưa audit lại toàn bộ. **T05/R09 còn IN_PROGRESS, chưa G2/nghiệm thu.**

**Điểm tiếp tục:** collector chỉ đọc với quyền tối thiểu, dữ liệu UNKNOWN theo từng tín hiệu, mốc tuổi hữu ích hơn và boot/sequence/nonce/clock thật, rồi nối màn dự phòng. Còn các nguồn lỗi M2 chưa đủ, M3/M5–M9, backup/restore, admission/readiness, lifecycle/WQ, các nhánh chức năng và gate cuối; writeReady=false. Chưa có bằng chứng độc lập máy/Human oncall/PROD/E1/T08/G2. Android/iOS Future chưa roadmap; Windows/Apple Silicon chưa kiểm nguồn mới; R10 chưa mở. Task kế tiếp khó: đề xuất **GPT-5.6 Sol + High** theo đánh giá phạm vi, [OpenAI Docs](https://developers.openai.com/api/docs/models/gpt-5.6-sol). [Bằng chứng/giới hạn](docs/R09_T05_PROGRESS.md). Các đoạn dưới giữ lịch sử.
...
Hiện hành R09-T05 là lát nguồn số đo thật/primary W5 ở đầu tài liệu và [báo cáo](docs/R09_T05_PROGRESS.md);IN_PROGRESS/review10DRAFT,chưa G2/nghiệm thu. Điểm tiếp tục collector/fallback/backup/readiness/WQ. Các mốc B/C/T02 và điểm tiếp tục cũ dưới đây là lịch sử,không dùng xin lại grant hoặc mở workload đã tiêu thụ.
