...
**Hiện hành — revision 13 đã được Human duyệt, 2026-09-20.** Human duyệt đúng `R09-T04-OUTPUT-r1` revision 13 với digest trình duyệt `52927f9e…f1475f2`. Public APPROVE trả `REVIEW_RECORDED`/`APPROVED`, byte verification đạt; digest toàn file sau duyệt là `2d1243ff…076abc`, giữ 10 owner, 117 subject version và 154 input version. Public READ mới trả WAITING và đọc lại đúng `APPROVED`; `W-009-ADMIN-OPS` vẫn IN_PROGRESS, không blocker và còn `IMPACT_REVIEW_REQUIRED`. Pilot commit local `993b7e8`, sạch/không remote/pending. [Bằng chứng](tests/evidence/r09/recovery-protocol-v3-r1/summary.json).

Approval nghiệm thu đúng gói revision 13, không tự hoàn tất T05/R09. Còn coverage PARTIAL/NOT_RUN, WQ/vận hành hữu hạn, semantic impact refresh, monitoring/observer, T08 restore/release, G2 và nghiệm thu cuối. Task kỹ thuật tiếp theo khó, đề xuất **GPT-5.6 Sol + High**. R10 chưa mở; Android/iOS Future, Windows/Apple Silicon chưa được chứng minh. Các đoạn dưới giữ lịch sử.
...
Kiểm lõi Kidea cuối trên Node `v24.21.0` đạt 293/293, không fail/cancel/skip/todo, stderr rỗng và đầu vào không đổi.

Public review hiện là revision 12 DRAFT; SAVE đã trả `CONTINUATION_SAVED`. `W-009-ADMIN-OPS` vẫn IN_PROGRESS, không blocker; impact ngữ nghĩa cũ còn cần làm mới. **T05/R09 còn IN_PROGRESS:** chưa scheduler/retention/restore drill, admission/readiness/lifecycle, coverage còn thiếu, WQ, G2/output acceptance/nghiệm thu. Điểm tiếp tục là admission/readiness/lifecycle rồi coverage/WQ; restore/release đầy đủ ở T08. Android/iOS Future, Windows/Apple Silicon chưa kiểm nguồn mới, R10 chưa mở. Task tiếp theo khó; đề xuất **GPT-5.6 Sol + High**. [Chi tiết/giới hạn](docs/R09_T05_PROGRESS.md). Các đoạn dưới giữ lịch sử.

**Lịch sử — checkpoint observer đã phục hồi (2026-09-19).** Human đã duyệt gói recovery r1. Áp đúng patch đã đóng băng: trần prepared-request 128 MiB, từng source/copy/checkpoint 64 MiB; chỉ chuyển đúng hai module theo hash cho operation `bef9054c`. Bản Kidea đang dùng qua **293/293 core + 9/9 recovery PASS**, không skip/cancel/todo. Public READ_RECOVERY mới rồi RECOVER trả **RECOVERY_COMPLETED**; work đúng bytes đã chuẩn bị, chỉ thay nextAction/checkpointRef, mọi owner/status/gate/history/review giữ nguyên. Marker được helper gỡ sau readback; request/tool identity gốc và journal quyết định còn nguyên. Pilot checkpoint local `aba73cf`, source observer `8779453`, clean/không remote. Observer 29 Mac/15 Docker Chrome là bằng chứng thành phần dùng lại, số đo/quyền/backup giả lập; backend/Web 132 HTTPS/22 SQL/bốn preset vẫn dùng lại. Coverage 137 giữ 11 covered/88 partial/38 NOT_RUN. Tiếp theo nguồn số đo/lỗi thật C++ và primary W5; còn backup/readiness/WQ/E1/Human oncall/T08/G2/nghiệm thu. T05/R09 IN_PROGRESS/DRAFT; native Future chưa roadmap, Windows/Apple Silicon chưa kiểm bản vá, R10 chưa mở. Task kế tiếp khó: GPT-5.6 Sol + High. [Bằng chứng và điểm tiếp tục](docs/R09_T05_PROGRESS.md). Các đoạn dưới giữ lịch sử.

**Lịch sử trước duyệt — observer PASS, checkpoint chờ phục hồi r1 (2026-09-18–19).** Observer chỉ đọc và màn dự phòng trong pilot nguồn `8779453` đã qua **29/29 kiểm Mac Intel và 15/15 nhóm Docker Chrome/TLS** trên nguồn cuối. Số đo, quyền và backup đều là dữ liệu giả lập; chưa nối backend thật hoặc màn vận hành chính W5. Backend/Web/core đang dùng không đổi; 132 HTTPS, 22 SQL, core 293 và bốn preset là bằng chứng dùng lại cho observer. Coverage 137 giữ 11 covered/88 partial/38 NOT_RUN, không nâng full OP. Public REVISE revision 9 DRAFT và READ WAITING; SAVE bị bộ điều phối 1200 s ngắt, marker `bef9054c` còn chờ, work BEFORE không đổi. Kidea tạo request 79,26 MiB vượt reader 64 MiB. Bản vá riêng đã qua 293 core/9 recovery và candidate chỉ đọc RECOVERY_READY; chưa áp vào bản đang dùng. Cần Human duyệt gói giới hạn 128 MiB, chuyển đúng hai module và hoàn tất đúng bytes ghi chú; giữ owners/gates/history và pilot HEAD 8779453. [Bằng chứng và điểm tiếp tục](docs/R09_T05_PROGRESS.md). T05/R09 vẫn IN_PROGRESS/DRAFT, chưa G2/nghiệm thu; native Future, Apple Silicon NOT_RUN, R10 chưa mở. Task khép phục hồi có độ khó cao: GPT-5.6 Sol + High. Các đoạn dưới giữ lịch sử.

**Lịch sử — observation/quyền truy cập T05 (2026-09-18).** HTTPS/WSS **132/132 PASS** giữ đủ 100 ca trước; SQL 22 checks, toàn bộ kiểm Web mới PASS. Sửa hai lỗi đọc cũ/đổi phiên giữa đọc và xác nhận ở nguồn45d3808; toàn bộ FAIL giữ riêng. Coverage137:11 covered/88 partial/38 NOT_RUN, 16 negative proof fixtures bị từ chối; ledger gốc không đổi. Backend112 files/artifact/bốn preset/EXr2/core293 dùng lại. Review8 DRAFT, public SAVE CONTINUATION_SAVED giữ owners/gates/history. T05/W-009-ADMIN-OPS IN_PROGRESS, chưa G2/nghiệm thu R09. Tiếp theo monitoring/observer/readiness rồi backup/WQ; các thiếu sót chức năng còn lại ghi trong coverage, release/restore ở T08. Native Future, Apple Silicon NOT_RUN, R10 chưa mở. [Bằng chứng và điểm tiếp tục](docs/R09_T05_PROGRESS.md). Các đoạn dưới giữ lịch sử.

**Lịch sử — xác nhận/hai form nhạy cảm/PAUSE T05 (2026-09-18).** HTTPS/WSS100/100PASS giữ đủ85kiểm cũ, SQL18checksPASS (66+18+56tracked admin intents/20+9+15workshops;17participant requests mới). Sửa lỗi STATE confirmation không hủy khi nguồn đã biết đổi ởb496e88; baseline2FAIL/2PASS giữ. Web93unit/18nativeSSR/4cũ+4mới/type/lint/build mớiPASS; backend112files/artifact/bốn preset/EXr2/core293 dùng lại. Coverage137:10covered/89partial/38NOT_RUN,9negative proof fixtures bị từ chối; không là sốtaskT05. Public review7 DRAFT, SAVE CONTINUATION_SAVED giữ gate/owner/history. T05/W-009-ADMIN-OPS IN_PROGRESS, chưaG2/nghiệm thuR09. Còn isolatedN-only/observation lỗi/ma trận quyền rồiops/backup/readiness/WQ; release/restoreởT08. NativeFuture, AppleSiliconNOT_RUN, R10chưa mở. [Bằng chứng và điểm tiếp tục](docs/R09_T05_PROGRESS.md). Các đoạn dưới giữ lịch sử.

**Lịch sử — publication failure/actor namespace/hai form T05 (2026-09-18).** HTTPS/WSS85/85PASS giữ đủ77kiểm cũ, SQL14checksPASS (66+18tracked intents/20+9workshops). Hai lỗi pilotWeb thật được sửa ở4544bd6; Web93unit/18nativeSSR/4regression/type/lint/build mớiPASS, backend/bốn preset/EXr2 và core293 chỉ dùng lại. Giữ mọiFAIL; không đổiquota/quyền/Kidea runtime. Coverage137:9covered/90partial/38NOT_RUN, không là sốtaskT05. Public review6 DRAFT, SAVE CONTINUATION_SAVED giữ gate/owner/history; T05/W-009-ADMIN-OPS IN_PROGRESS, chưaG2/nghiệm thuR09. Còn sensitive concurrency/PAUSE/ma trận quyền rồiops/backup/readiness/WQ; release/restoreởT08. NativeFuture, AppleSiliconNOT_RUN, R10chưa mở. [Bằng chứng và điểm tiếp tục](docs/R09_T05_PROGRESS.md). Các đoạn dưới giữ lịch sử.

**Lịch sử — kiểm mất phản hồi/nhấp đúp/hai thứ tự tranh chấp T05 (2026-09-18).** HTTPS/WSS **77/77 PASS**, giữ đủ71kiểm cũ; SQL audit66intents/20workshops PASS. Thêm6nhóm, giữ UNKNOWN qua reload/GET khi CREATE/EDIT chưa tới backend, không POST thay thế/tìm tên; nhấp đúp cả3action ghi một lần; hai thứ tự C10,N9→C9 giữ đúng dữ liệu. Ứng dụng/backend112nguồn/Web build/dependencies và Kidea runtime không đổi; build/unit/core cũ dùng lại, không lượt mới. Hai FAIL thuộc đường bộ kiểm, raw giữ riêng; auditor cuối từ chối2fixture thiếu proof và xử lý path relative/absolute tương đương. AD-T04/AR-T06 có chứng cứ đủ chức năng; ma trận137toàn pilot vẫn5covered/94partial/38NOT_RUN, không là số task T05. [Bằng chứng/giới hạn/điểm tiếp tục](docs/R09_T05_PROGRESS.md). Public review revision5 DRAFT; T05/W-009-ADMIN-OPS IN_PROGRESS, chưa G2/nghiệm thu R09. Còn publish failure/absent STATE/actor namespace và vận hành/tải; release/restore đầy đủ ởT08. Quyền trọn R09 giữ nguyên; native Future, AppleSilicon NOT_RUN, R10 chưa mở. Các đoạn dưới là lịch sử.

**Lịch sử — audit ma trận T05 và regression quản trị (2026-09-18).** Đã đối chiếu đủ137 nghĩa vụ nguồn, giữ4ca Max2 riêng và mọi phần thiếu; ledger gốc nguyên byte. Sửa thông báo lịch sử/lỗi capacity khi registration thắng cuộc đua giảm sức chứa. Web93unit/18SSR–Chrome, HTTPS/WSS backend thật **71/71 PASS** giữ đủ62ca cũ, SQL audit sau shutdown PASS. Backend112nguồn/artifact cũ không đổi; EXr2 giữ giới hạn, không build backend mới. [Bằng chứng/FAIL/checkpoint/việc còn lại](docs/R09_T05_PROGRESS.md). Public revision5 DRAFT, T05/W-009-ADMIN-OPS IN_PROGRESS; chưa G2/nghiệm thu R09. Tiếp theo các nhánh admin failure/concurrency/actor namespace còn thiếu, rồi ops/backup/restore/tải. Quyền trọn R09 giữ nguyên; Android/iOS Future, Apple Silicon NOT_RUN, R10 chưa mở. Các đoạn dưới là lịch sử.

**Lịch sử — lát danh sách trực tiếp và `/me` (2026-09-18).** Đã nối cập nhật trực tiếp danh sách public/admin và `/me`, thêm xác minh identity/generation và bảo vệ lịch sử đã hủy. Web93 unit,18 SSR–Chrome Mac PASS; HTTPS/WSS backend thật **62/62 PASS** trên nguồn cuối, không bypass TLS. Backend giữ đúng artifact bốn preset đã kiểm:112 nguồn không đổi, EX r2 SQLite/WAL cũ giữ giới hạn. [Bằng chứng, FAIL giữ nguyên và việc còn lại](docs/R09_T05_PROGRESS.md). W-009-ADMIN-OPS vẫn IN_PROGRESS; review nguồn là DRAFT, chưa G2/nghiệm thu R09. Tiếp theo đối chiếu và bổ sung ma trận T05 còn thiếu, rồi vận hành/tải/backup/restore. Quyền trọn R09 giữ nguyên; Android/iOS Future chưa roadmap, Apple Silicon NOT_RUN, R10 chưa mở. Các ghi chú tiến độ phía dưới là lịch sử.

**Lịch sử trước nghiệm thu T04, 2026-09-18:** dev/ASan/release đủ 50case/856assertion; TSan 43case sạch +7case đúng EXr2 (14report), raw FAIL giữ nguyên. Web34unit/18SSR–Chrome và HTTPS18kiểm PASS;4mutant bị phát hiện. Ba bản đồ cập nhật nhưng vẫn INCOMPLETE; chưa G2/137nghĩa vụ hoặc nghiệm thu R09. [Gói cụ thể](docs/R09_T04_REVIEW.md). Các ghi chú “đang chạy” bên dưới là lịch sử.
...
**Môi trường cập nhật:** Human đã cấp quyền GCP `kidea-508908`; [lab độc lập đã PASS và thu hồi sạch](tests/evidence/r08/cloud-execution-r1.md). [Quyền/giới hạn](docs/R08_GCP_AUTHORITY.md) giữ nguyên; số dư credit/hạn và hóa đơn thực chưa xác minh. Không cần Human cài thêm hoặc tự dọn VM cho lượt này.

<a id="review-current"></a>

Hiện hành R09-T05 theo [báo cáo](docs/R09_T05_PROGRESS.md): `W-009-ADMIN-OPS` IN_PROGRESS; `R09-T04-OUTPUT-r1` revision 13 đã được Human duyệt và public READ đọc lại `APPROVED`, nhưng chưa G2/nghiệm thu T05/R09. Điểm tiếp tục là coverage reconciliation, WQ/vận hành hữu hạn và semantic impact refresh; monitoring/observer và T08 restore/release vẫn mở. Các mốc B/C/T02 và điểm tiếp tục cũ dưới đây là lịch sử, không dùng xin lại grant hoặc mở workload đã tiêu thụ.

**Lịch sử — B/C đã nghiệm thu, T02 authoring đã viết/kiểm tĩnh:** Human “Tôi duyệt” cho [gói tại cdd6ee5](proposals/r09-t02-authoring-r1.md), chấp nhận B/C tại60d0a33 và mở viết source/test/harness. [Kết quả T02](tests/evidence/r09/t02-authoring-r1.md): backend/contracts/46nhóm C++ + HTTP test,137ID vẫn NOT_RUN;9kiểm tĩnh qua, chưa compile/container. Pilot source db7b7ef, local HEAD301d349 sạch/không remote;21docs/7906sample entries và168nguồn Kidea bảo toàn. [Build r1 cụ thể](docs/R09_T02_BUILD_REVIEW.md) có51sourcehash/824vendorfile, chưa duyệt chạy.

**Điểm tiếp tục tại mốc lịch sử:** duyệt đúng gói build r1 (2giờ/4container tuần tự/2CPU4GiB/8GiB disk/2GiB data-log/0download/networknone/nohostport), rồi mới chạy. Không xin lại B/C/D/E hoặc nghiệp vụR03–R05. T02/R09 chưa DONE, R10 chưa mở; các đoạn B/C IN_REVIEW bên dưới là lịch sử trước approval tại cdd6ee5.

**Lịch sử — R09 A–E APPROVED:** Human “Tôi duyệt tất cả mục trên nhé” cho [gói tại 77f5caf](proposals/r09-next-decisions-r1.md). [A nghiệm thu D1](docs/R09_D1_ACCEPTANCE.md) tại26dffeb; S02 DONE. B1–B3 đã triển khai và [kiểm nguồn cuối](tests/evidence/r09/ae-r1.md): 9/9 bộ PASS, core 293/293, 17 ca mới, R06/R07/R08 local; 168 file nguồn không đổi. B/C IN_REVIEW, chưa nghiệm thu đầu ra. C đã chuẩn bị hồ sơ, Git local/public init/SAVE ở sibling, WAITING tại W-001, không remote; chưa code/build/deploy workshop hoặc AI trial. D/E đã chốt chính sách theo thời điểm; không hỏi lại ý nghĩa nghiệp vụ/lab.
...
**Đã review bằng chứng Android A1 và chuẩn bị [gói thiết bị/iOS r1](proposals/r05-native-device-r1.md).** Kiểm lại23file nguồn snapshot/live,21hồ sơ pilot, hai suite11ca, lint và sáu mutant đạt đúng phạm vi evidence review; không chạy workload mới. Human xác nhận không có điện thoại Android: device BLOCKED_DEVICE_UNAVAILABLE, không tự cài emulator. iOS tiếp tục chuẩn bị trên Mac Intel hiện tại; Apple Silicon để sau/NOT_RUN. Cần hoàn thiện gói OS/Xcode, backup và manifest/quyền/ngân sách trước cài. ADB host, kết nối/cài APK, nâng macOS/Xcode chưa được thực thi. A2/A3 chưa INSTALL_READY; R05 vẫn IN_PROGRESS, Apple Silicon NOT_RUN. Các đoạn sau ghi kết quả và lịch sử A1.
...
**R5 đã hoàn tất các vector lab backend/Web còn thiếu; chờ review kết quả, chưa khép R05.** Human duyệt ngoại lệ hẹp R05-TIDY-01 và tiếp tục sau `9b93e10`. [Báo cáo r5](tests/evidence/r05/backend-execution-r5.md): core Mac Intel 283/283; clang-tidy PASS; mỗi preset C++ dev/ASan+UBSan/TSan/release 12/12; sáu mutant bị bắt; backend thật idle/active/stuck dưới dev/ASan/TSan đạt. HTTPS 38, upstream 15, Chromium HTTPS 8/8 hai lần và edge drain cuối đều PASS. Nguồn/artifact Caddy và Web không đổi được đối chiếu với evidence r4; không nhận các bài đó là lượt chạy mới r5.

Guard quota đã được kiểm bằng Docker thật và chặn client vượt trần trước khi tạo; bootstrap NSS mới chạy lặp đạt. R5 giữ 2 CPU/4 GiB, delta đĩa tích lũy khoảng 19/24 GiB, không tải/cài mới; tất cả container dừng, 8443 đóng. [Sự cố quota r4](tests/evidence/r05/backend-execution-r4/resource-incident.md) và các FAIL vẫn giữ nguyên. [Ngoại lệ R05-TIDY-01](proposals/r05-sqlite-transient-tidy-r1.md) **APPROVED/applied**, chỉ một macro; tham số trim được sửa thông thường, không suppression thêm. Điểm tiếp tục: review phạm vi backend/Web rồi gói môi trường/mẫu Android/iOS; chưa có quyền cài SDK/nâng OS hoặc tự nghiệm thu. R05/T02-S03/T03-S03 IN_PROGRESS (lab evidence ready for review), native và T07-S02 còn mở; Apple Silicon NOT_RUN. Các đoạn dưới là lịch sử.

**Đã duyệt và thực thi thay dependency Caddy r1; hiện chặn bởi tương thích source.** [Lượt r2](tests/evidence/r05/backend-execution-r2.md): Go 1.26.7 chạy trong Docker, module resolve/checksum PASS; build Caddy FAIL tại hai lời gọi API CEL đã đổi. [Bản vá hai vị trí](proposals/r05-caddy-cel-compat-r1.md) sẵn để review, chưa áp. C++ dev và ASan/UBSan đều 9/9 unit PASS sau thêm bounded queue; chưa TSan/release/mutation/Web Docker/HTTPS/full matrix. Tải browser bị hủy có log vì nhánh HTTPS chặn; container dừng, port 8443 đóng. R05/T03-S03 IN_PROGRESS, Apple Silicon NOT_RUN. Các đoạn dưới là lịch sử.

**E2-BW-r1 đã được duyệt và chạy một phần; BLOCKED_DEPENDENCY_REVIEW.** [Kết quả Docker](tests/evidence/r05/backend-execution-r1.md): toolchain PASS, C++ dev 8/8 unit PASS, giữ log FAIL/retry. Image Caddy dùng Go 1.26.3 có advisory TLS áp dụng nên chưa mở HTTPS. [Đề xuất thay Caddy](proposals/r05-caddy-remediation-r1.md) chờ duyệt trong cùng giới hạn Docker, không cài host. Sanitizer/release/Web Docker/HTTPS/mutation còn NOT_RUN; chưa full matrix. Container/builder dừng, port 8443 đóng; 27 file Web r2 và 21 hồ sơ pilot nguyên hash. R05/T03-S03 IN_PROGRESS, Apple Silicon chưa kiểm. Các đoạn dưới là lịch sử.

**Gói E2-BW-r1 đã chuẩn bị, chờ duyệt thực thi.** [Backend/HTTPS r1](proposals/r05-backend-build-r1.md) có4image digest,4source archive hash,16gói APT trực tiếp/snapshot và22nhóm vector; [kiểm tĩnh](tests/evidence/r05/backend-plan-r1/preparation-verification.json) PASS đúng phạm vi chuẩn bị. Đề xuất SQLite3.53.4/cmark0.31.2; Caddy2.11.4 giới hạn cấu hình không forward_auth vì advisory ghi bản vá2.11.5 nhưng release chưa lấy được. Trần4GiB tải/16GiB đĩa/2CPU4GiB/3giờ, chỉ loopback8443, CA riêng trong container. Chưa pull/build/run, APT transitives/signature và runtime vẫn NOT_RUN. R05 IN_PROGRESS; chưa nhận approval thực thi E2 hoặc nghiệm thu đầu ra. Các đoạn dưới là lịch sử.
...
**R05 đã được duyệt quyền soạn/kiểm và có hồ sơ r1 chờ duyệt nội dung.** Human “Tôi duyệt nhé” sau [answer cfffe0c](https://github.com/Kynderis/kidea/blob/cfffe0c/answer.md) chấp thuận D1–D6/P1–P4, chuyển đích sang Mac và khôi phục pilot khi thiếu. [Kết quả r1](tests/evidence/r05/profile-r1.md): khôi phục đúng 15 file, thêm sáu hồ sơ; 40 rule, 20 nhóm test kỹ thuật nối 137 ca nguồn; 998 link nội bộ hợp lệ. R05 doc checks **11/11 PASS**, chạy kèm R03 cũ **8/9** do inventory chưa nhận engineering: tổng **19/20, giữ FAIL**, không sửa test cũ để xanh. Nguồn R03/R04 bảo toàn byte; chỉ hai backlink được nối. T01-S02/S03, T02–T06-S01 và T07-S01 DONE đúng phạm vi soạn/kiểm/báo cáo; **T01-S04/T02–T06-S02 IN_REVIEW cho R5-1–R5-4** trong [phương pháp](proposals/r05-profile-method-r1.md). Chưa tích hợp skill, cài/build/code/deploy hoặc mở AI mới; build/sample vẫn NOT_RUN và là nghĩa vụ R05. Gói môi trường chưa đủ exact artifact/dung lượng/quyền để xin cài; Xcode26.6 không phù hợp macOS14.7 hiện tại. LP-01 vẫn IN_PROGRESS, Intel core đã PASS đúng nguồn, Apple Silicon NOT_RUN; không chặn review hồ sơ R05. Các đoạn dưới là lịch sử.

**Mac Intel đã kiểm lõi LP-01: 283/283 PASS; Apple Silicon còn NOT_RUN.** [Bằng chứng Mac ngày 2026-09-16](tests/evidence/local-portability-mac-intel-2026-09-16.md) ghi macOS 14.7/APFS local, Intel x64, Node 24.19.0 có sẵn, npm 12.0.2 và Git 2.54.0; `npm ci` đạt, lockfile không đổi. Checkout `Kynderis/kidea`/`master` tại `b1f8890` sạch lúc bắt đầu; 36 file nguồn/test/metadata khớp Windows và không đổi trước–sau lượt kiểm. Không fail/skip/cancelled/todo, không cần sửa helper/test; giữ nguyên mọi evidence Windows. Node mặc định trên PATH vẫn là 22.22.2; phiên này gọi trực tiếp Node 24 có sẵn, không cài hoặc đổi cấu hình máy. **LP-01 vẫn IN_PROGRESS, còn kiểm Apple Silicon thật khi có máy/quyền.** R05-T01-S02 vẫn IN_REVIEW cho [D1–D6/P1–P4](proposals/r05-profile-test-r1.md), chưa thực hiện gói đó. Điểm tiếp tục: giữ kết quả Intel đúng nguồn này, kiểm Apple Silicon bằng cùng suite khi có điều kiện; R05 chỉ đi tiếp sau đúng gate. Docker/cloud/iOS, build ứng dụng và pilot không được kiểm trong lượt này. Các đoạn dưới là lịch sử.

**LP-01 đã hoàn tất phần mã/tài liệu và kiểm Windows; còn kiểm Mac thật.** Human duyệt đề xuất sau [answer f8b47b9](https://github.com/Kynderis/kidea/blob/f8b47b9/answer.md), chỉ cần thư mục local. Đã bỏ khóa Windows/NTFS, exact Node binary và Git path; giữ an toàn ghi/quyền/approval/pending, cùng helper trên Windows/macOS Intel/Apple Silicon. [Báo cáo kiểm chứng](tests/evidence/local-portability-r1.md): 283/283 ca trên mỗi Node 24.21.0 và 24.19.0, cùng nguồn không đổi; 24/24 kiểm tài liệu R03/R04, 826 link pilot, validator PASS. Amendment Docker local/cloud chỉ đổi môi trường, giữ nguyên byte nền R04. LP-01 IN_PROGRESS **chỉ còn xác nhận runtime Mac Intel/Apple Silicon thực, hiện NOT_RUN vì chưa có máy/kết nối**; không xin duyệt lại các sửa đổi thường lệ đã nằm trong gói. [Một lệnh kiểm trên máy đích](tests/local-portability/README.md). Không cài Docker/SDK, build ứng dụng hay kết nối cloud; mạng/iCloud/OneDrive ngoài phạm vi, không là việc để làm sau. R05 vẫn mở chuẩn bị, D1–D6/P1–P4 chưa được duyệt toàn bộ; không suy approval gói này thành khép R05. Các đoạn dưới là lịch sử.
...
**AI trial r1 đã dùng 3/6 phiên, chưa đủ khép R02.** Human duyệt gói sáu phiên sau answer db184fa; [báo cáo](tests/evidence/r02-t10-agent-trial-r1.md) ghi 3 PASS/3 FAIL/6 PARTIAL/30 NOT_RUN trên 42 ô dự kiến. Cả ba A lộ lỗi spelling root ở request đầu; đã sửa hướng dẫn bằng Resolve-Path/realpath, test mới 1/1 và hồi quy chức năng 22/22 đạt, runtime giữ nguyên. Không có SAVE handoff nên ba B không chạy. Cutoff điều phối được áp dụng bảo thủ từ manifest thay vì từng spawn, có hạn chế đo thời gian và A1 kết thúc lệnh muộn 2 giây; không nhận timing PASS. Cần Human duyệt thay gói còn lại thành sáu phiên mới ×10 phút (tổng chín kể cả ba đã dùng) trước thử AI lại. Chưa mở R03 hoặc sửa bảo mật Windows. Các kết quả phía dưới giữ phạm vi/bản lịch sử.

**Bài thử chức năng thu gọn đã chạy đạt 22/22 tình huống qua 24 public CLI calls; hồi quy review/resume đạt 60/60.** Human ngày 2026-09-15 đồng ý làm trên project giả ở D, không VM/sandbox setup hoặc thay tài khoản/quyền Windows/firewall, và yêu cầu làm liền mạch trong quyền thay vì hỏi lại từng bước. [Báo cáo và bằng chứng](tests/evidence/r02-t10-functional-flow.md) giữ cả lần đầu lỗi kỳ vọng của test (14 ca đạt trước khi dừng) và lần sau đạt đủ; không sửa runtime để đổi kết quả. Đây là kiểm thử chức năng bằng script dưới phiên điều phối hiện tại, không phải sáu phiên AI độc lập hoặc kiểm chứng cách ly OS. Các snapshot launcher phía dưới là lịch sử, không còn là prerequisite của bài thử chức năng này. Phần AI độc lập, change/visualize và nghiệm thu toàn bộ R02 vẫn chưa hoàn tất.

**Launcher đã đổi sang 0.154.0-alpha.6.2; bản/path 0.153.4 không còn hiện hành.** [Kiểm tra giao diện bản mới](tests/evidence/r02-t10-launcher-0154-interface.md) xác minh hash, help và xuất schema trên D, không chạy server/sandbox/model. Giao diện được kiểm không có preview danh sách ACL của setup; chưa chứng minh ngoại lệ chỉ-fixture. Điểm cần Human quyết là có chấp nhận setup sandbox tiêu chuẩn trên máy ngoài fixture hay giữ giới hạn cũ; chưa cấp ngoại lệ hoặc phiên AI mới. Các đoạn snapshot phía dưới là lịch sử đúng lúc kiểm.

**Đã kiểm tra thêm máy hiện tại theo yêu cầu sau 70562b5:** C trống 17,86 GiB, D 236,32 GiB; account/firewall sandbox đã có. ACL hiện tại có entry đọc cho nhóm sandbox ở Documents/Downloads/`.codex`; không chứng minh agent đã đọc chúng. CLI help xác nhận profile riêng, nhưng chưa chứng minh giới hạn setup/đọc thực tế. [Kết quả và phạm vi còn thiếu](tests/evidence/r02-t10-native-host-check.md). Chưa chạy sandbox/model, đổi ACL/config, tạo VM hoặc cấp quota; không trình danh sách ACL chưa đầy đủ như gói READY.
...
Human **“Duyệt nhé”** sau answer tại `8aad5d2297195400a2cc3421af5654185e2531b3` đã duyệt D1 ngày 2026-09-15. Mô tả IN_REVIEW phía trên là lịch sử bản trình; hiện được chuẩn bị/đo theo đúng D1. Chưa có approval ngưỡng, sáu phiên AI dự kiến hoặc quyền launcher.

Human **“Ok làm đi”** sau answer tại `3e142f7548bbb9f0bab08975f82e11a13e6a7f87` duyệt [D2](proposals/r02-t10-probe-r2.md) ngày 2026-09-15: 22 lượt mới sau tối ưu, fixture byte/expected giống D1, chỉ root `probe-r2`, không retry/bù; giữ nguyên evidence D1. Không thêm tối ưu runtime trong loạt khóa và không duyệt ngưỡng hoặc phần AI.

Phần AI còn DRAFT: dự kiến ba lượt lặp × hai phiên tách lịch sử để kiểm init/review rồi resume, 14 biến thể mỗi lượt; chưa cấp sáu phiên, model hoặc launcher. Chỉ trình quyền chạy khi manifest thực và ranh giới launcher đã xác minh. T10-S03 chốt ngưỡng sau đo, T10-S04 chạy nghiệm thu mới, T11-S03 Human review khép R02; không mở R03/pilot trước các gate. Lượt trình ban đầu chỉ thay tài liệu. Sau D1 đã chuẩn bị/đo thăm dò theo [báo cáo](tests/evidence/r02-t10-probe.md), không chạy AI, ACL hoặc sandbox.
...
### R02-T06-S04 — tinh gọn mô hình một lượt sửa, đã duyệt hướng

Human **“Tôi đồng ý hướng bạn đề xuất”** trong hội thoại ngày 2026-09-14 sau rà soát tổng quan–chi tiết. [DESIGN hiện hành](KIDEA_DESIGN.md#lean-operation-approved): một Human/một lượt hợp tác; Git-first đúng byte và giữ bản, snapshot tối thiểu; dấu bận/hồ sơ ghi dở/readback; không tự replay/restore sau gián đoạn. Thay bảo đảm Win32 chống editor, không mở thêm quyền môi trường.

- Lượt này đồng bộ DESIGN/ROADMAP/KA/KQ, thay writer/caller liên quan và kiểm thử xác định trên dữ liệu tổng hợp. Giữ evidence/protocol/manifest cũ đúng bản; không sửa expected cũ để lấy PASS mới.
- Giữ 10 bước sản phẩm/Human gate, 6 hành động, ba góc nhìn bản đồ, G2 toàn Feature, pilot thật/production và đồng thời nghiệp vụ. Hồ sơ/subtask theo nhu cầu; hỏi quyết định đáng kể; N/A một lần đúng phạm vi.
- Task xây Kidea phụ thuộc đầu vào thực, không chỉ số thứ tự. T07 AI chưa chạy không blanket-block chuẩn bị/triển khai phần T08/T09 đủ hợp đồng/primitive/quyền; prerequisite thật và gate tích hợp vẫn chờ. Không tự đổi gate sản phẩm.
- Gom AI trial theo luồng có ý nghĩa, case/quota/phạm vi/quyền rõ trước chạy. Approval này không mở phiên AI hoặc dùng quota/manifest cũ cho bản mới. ACL incident riêng; VM không là mặc định.

Kết quả triển khai/kiểm chứng mới ở [bằng chứng S04](tests/evidence/r02-t06-cooperative.md) và dòng S04 trong sổ. PASS đến từ lượt kiểm thử mới, không suy từ approval; R02/pilot chưa hoàn tất.
...
Human “Còn phần trên tôi duyệt nhé” sau [answer 38daef8](https://github.com/Kynderis/kidea/blob/38daef8294d0123cf96e0bf213bd2b95a6f2d20f/answer.md) đã duyệt quota/phạm vi này. Không hỏi lại quota, D1/S01 hoặc xin nghiệm thu kết quả chưa có. Human “Ok hiểu rồi. Tôi duyệt phần ACL phía trên nhé” đã duyệt ngoại lệ chỉ trong cây dữ liệu giả tại bản 86f15c8. [Preflight và sự cố](tests/r02-t07/launcher-preflight.md): gọi nhầm launcher khi xem help đã kích hoạt read-ACL helper từ root repo; chưa có baseline trước để xác định ACL đổi. Dừng mọi sandbox/AI tiếp theo, không tự sửa ACL/config hoặc rollback đoán. Protocol/manifest bản trình giữ nguyên để đối chiếu; xác nhận mới nằm tại sổ này.
...
### R02-T06-S03-trial-r1 — quyền đã duyệt, quota đã dùng hết

Sau khi ghi bị ngắt, AI phải báo đúng phần còn dở, không tự phục hồi hoặc nói đã xong. Human “Ok nhé” sau [answer cfa374a](https://github.com/Kynderis/kidea/blob/cfa374a9b5ebc870ba79eafbc44afad6f1703d34/answer.md) đã cấp **3 phiên AI mới độc lập**, mỗi phiên đọc cùng 3 hồ sơ giả cố định: ghi byte xong nhưng công việc còn dở; UPDATE bị ngắt; cleanup bị ngắt sau khi xóa một bản tạm. Chỉ dùng skill/status chỉ đọc, không cho AI gọi helper ghi nội bộ.
...
[Protocol, fixture và cách chấm](tests/r02-t06/ai-trial-protocol.md). Đã thực hiện đủ ba phiên, cả 9 báo cáo case đạt, không timeout/đổi monitored bytes/hành vi ngoài quyền được quan sát; [kết quả và đối chiếu KA-10](tests/evidence/r02-t06.md#ai-trial-result). Không dùng lại quota hoặc nhận toàn KA-10 PASS; caller và tích hợp còn phải kiểm. Giữ raw/cảnh báo/counterexample cũ; không thêm gate duyệt kết quả T06 khi các phần [A] đã đủ trong quyền.

<a id="safe-write-r2-approved"></a>

### R02-T06-S01-r2 — lịch sử, đã được thay bằng mô hình một lượt hợp tác
...
### R02-T06-S01-r1 — lịch sử cơ chế khóa Windows, không còn hiện hành
...
Human “Tôi duyệt nhé” sau [giải thích bd52a52](https://github.com/Kynderis/kidea/blob/bd52a5244dd7d2697fddea823807297ac046a166/answer.md) đã duyệt D1–D2 gói d687d6a. R2 từng thay hai điều kiện D1; cả r1/r2 nay giữ lịch sử và được thay bằng [mô hình hợp tác](#lean-core-review), không còn nghĩa vụ khóa Windows. Kết quả/protocol cũ giữ nguyên; không mở AI/project thật từ đoạn lịch sử này.
...
- Chỉ một subtask triển khai hiện hành; công cụ sửa nguồn cùng project tuần tự trong một lượt hợp tác. Review độc lập chỉ đọc có thể hỗ trợ. Dùng prerequisite đầu vào thật, không số thứ tự; blocker bài thử không tự đóng việc độc lập. Không cố định số task như cam kết thời gian/phần trăm.
...
| R02-T06-S01 | DONE | Lịch sử r1/r2 đã APPROVED; được thay bằng S04 | Giữ xác nhận Human sau bd52a52/d687d6a và 5466a42, [lịch sử](#safe-write-review). Mô hình hiện hành tại [S04](#lean-core-review), không coi các bảo đảm khóa cũ còn bắt buộc |
...
| R02-T06-S04 | DONE | [A] — hướng tinh gọn APPROVED ngày 2026-09-14 | Đồng bộ 4 tài liệu, Node writer/Git-first và caller init/status. 174/174 = 15 helper + 78 status + 61 writer + 20 init, nguồn/đầu vào không đổi; [bằng chứng/lỗi/giới hạn](tests/evidence/r02-t06-cooperative.md). Skill validator/syntax đạt; native được bỏ khỏi runtime, lịch sử giữ qua Git. Không AI/sandbox/ACL/VM/project thật; chưa nghiệm thu toàn lõi |
...
| R02-T07-S03 | DONE | [A] — hành vi init kiểm bằng gói AI r2 riêng | Ba A init đúng/thiếu quyền/init lặp đạt; [kết quả](tests/evidence/r02-t10-agent-trial-r2.md). Quota launcher cũ vẫn 0/3, sự cố giữ tại [preflight](tests/r02-t07/launcher-preflight.md), không chuyển quota hoặc nhận đã sửa sandbox |
...
| R02-T08-S03 | DONE | [A] — gói AI r2 đã được cấp riêng | Ba phiên A review/feedback/approval, ba B giữ/mở lại review; [kết quả và lỗi giữ lại](tests/evidence/r02-t10-agent-trial-r2.md), không tái dùng quota T07 |
...
| R02-T09-S03 | DONE | [A] — gói AI r2 đã được cấp riêng | Ba bàn giao A→B, SAVE/thiếu evidence/pending/UNKNOWN; [kết quả](tests/evidence/r02-t10-agent-trial-r2.md), không diễn tập PROD hoặc cross-machine |
| R02-T10-S01 | DONE | [H] — D1–D4 và AI r2 đã được cấp | Human duyệt điều chỉnh sau dc3898c; [manifest/quota/protocol r2](tests/evidence/r02-t10-agent-protocol-r2.md). Không thêm lượt sau sáu phiên mới |
| R02-T10-S02 | DONE | [A] — giữ số đo và mọi lượt AI | D1/D2/D4 mỗi loạt 22, D3 một profile giữ evidence; AI r1 giữ lỗi và phần chưa chạy, [r2](tests/evidence/r02-t10-agent-trial-r2.md) đủ sáu phiên, 104 calls |
| R02-T10-S03 | DONE | [H] — chuẩn status và gói AI được duyệt trước chạy | [Chuẩn status](proposals/r02-t10-status-acceptance-r1.md) 2/5 giây; [AI oracle r2](tests/evidence/r02-t10-agent-protocol-r2.md) giữ 14 tình huống/cặp, chỉ điều chỉnh số phiên/thời gian được Human cấp |
...
| R03-T01-S02 | DONE | [H] — gói docs r1 APPROVED sau ab75b09 | Đã tạo đúng hai file; không `.kidea`, Git/code/deploy; [bằng chứng](tests/evidence/r03/document-trial-r1.md) |
...
| LP-01 | IN_PROGRESS | [H] — hướng và triển khai APPROVED sau f8b47b9 | [Gói bổ sung local/đa nền tảng](proposals/local-portability-r1.md): Windows và [Mac Intel 283/283 PASS](tests/evidence/local-portability-mac-intel-2026-09-16.md) trên cùng 36 file; Apple Silicon NOT_RUN. Docker local/cloud chưa cài/chạy; hồ sơ R05 r1 đang IN_REVIEW |
| R05-T01-S02 | DONE | [H] — D1–D6/P1–P4 APPROVED sau cfffe0c | Đích Mac/pilot sibling, soạn và kiểm nhẹ; không quyền cài/build |
| R05-T01-S03; R05-T02–T06-S01 | DONE | [A] — bản đề xuất r1 | Hợp đồng, bốn profile và test specs; từng hồ sơ giữ gate nội dung riêng trong gói R5-1–R5-4 |
| R05-T07-S01 | DONE | [A] — đã kiểm và báo đúng giới hạn | [R05 r1](tests/evidence/r05/profile-r1.md): 11/11 R05, 8/9 R03 cũ; không nhận mọi suite PASS hoặc phase DONE |
| R05-T01-S04; R05-T02–T06-S02 | DONE | [H] — nội dung R5-1–R5-4 tại a39f79f APPROVED | [Gói môi trường/build](proposals/r05-environment-build-r1.md) chờ quyền riêng; chưa cài/build |

R01–R04 đã APPROVED đúng phạm vi; R04 và R05 có phân rã/evidence được dẫn ở mục phase tương ứng. Hồ sơ R05 r1 chờ duyệt nội dung, LP-01 còn kiểm Apple Silicon, không thêm phase sản phẩm hoặc nghiệm thu lại R03/R04 từ đầu. Quota launcher T07 cũ vẫn 0/3, AI r2 hết sáu phiên, không chuyển quota. Bằng chứng cũ giữ đúng phiên bản; quyền mới chỉ theo gói hiện hành, không suy ra cài/build/deploy.
...
<a id="r02-t04-s01-result"></a>

### Kết quả R02-T04-S01 — ngày 2026-09-12

- Human “Ok tôi hiểu rồi. Duyêt nhé” sau hai lượt giải thích trong hội thoại về [gói D1–D2 tại e463a7b](https://github.com/Kynderis/kidea/blob/e463a7b2910349fb4e704b69bc1ffa63e2bd6983/answer.md). Giải thích thứ nhất: giữ bản đã xem, phát hiện file đổi rồi AI xét ý nghĩa. Giải thích thứ hai: lưu đúng nội dung lúc trình; lời duyệt chỉ gắn với bản đó khi Human xác nhận, không tự APPROVED khi lưu và không có hai tài liệu hiện hành cần Human quản lý.
- Đồng bộ [nguồn hợp đồng](KIDEA_DESIGN.md#source-version-contract): bản byte của file cần thiết được phép lưu, SHA-256 phát hiện khác biệt và đối chiếu nội dung/đầu vào. Giữ giới hạn dữ liệu nhạy cảm, chuyển máy, mã không thay ngữ nghĩa/quyền và không là backup chống hỏng ổ đĩa.
- Rà caller T03, schema r1, G4/resume, KA-07/10–13 và KQ-10; không đổi AC/QUALITY/G2 hoặc nhận schema/snapshot/runtime mới đã có. S05/S06 và cơ chế ghi T06 còn gate riêng; bộ mẫu T02/T03 giữ nguyên. Không chạy test mới để nhận các hành vi này đã đạt.
- Kiểm tra tài liệu/link/trạng thái và cleanup; chỉ sửa DESIGN/ROADMAP/answer, không tạo scratch, xóa dữ liệu, sửa helper/skill, cài dependency hoặc chạy pilot. Ngoại lệ hai thư mục rỗng T01 giữ nguyên.
- Khép S01, chỉ mở S02 [H] để trình căn cứ trước ghi/cách đối chiếu khi ngắt. T04 chưa khép; S03/S04 chưa bắt đầu, chờ đủ hợp đồng liên quan.

<a id="interrupted-write-review"></a>
<a id="r02-t04-s02-result"></a>

### Kết quả R02-T04-S02 — ngày 2026-09-13

- Human “Ok, Tôi duyệt cả 2 nhé” sau [gói D1–D2 tại 24a3b61](https://github.com/Kynderis/kidea/blob/24a3b61a66d7024b6480aeeb7ca74a23f85a0222/answer.md) và hai phần làm rõ trong hội thoại: (1) tận dụng Git nếu có đúng nội dung trước thao tác, giữ riêng phần chưa commit, không commit từng lần sửa; (2) tự dọn bản tạm do Kidea tạo khi việc xong/kiểm tra đạt/không còn cần phục hồi, giữ bản làm căn cứ đã duyệt và bản còn cần cho lỗi/ghi dở. Đây là nội dung xác nhận mới có cả phần làm rõ, không nói toàn bộ đã nằm trong commit cũ.
- Đồng bộ [hợp đồng](KIDEA_DESIGN.md#interrupted-write-contract): căn cứ ngay trước ghi, đối chiếu từng đích sau gián đoạn, không đè sửa ngoài luồng hoặc replay tác dụng phụ. Git phải truy được đúng byte cần phục hồi; không lấy commit cũ làm mất việc chưa commit. Cleanup theo danh sách/đường dẫn/quyền và nhu cầu thực, không xóa lịch sử/evidence bắt buộc hoặc làm mất căn cứ approval.
...
Lộ trình giữ **83 ID task** để truy lịch sử; **77 task trong phạm vi hiện hành**, **6 ID native chuyển Future chưa roadmap** theo [quyết định phạm vi](KIDEA_DESIGN.md#client-web-scope-approved). Không đánh số lại hoặc nhận6task là DONE. Số lượng không là thời gian hoặc phần trăm hoàn thành; phân rã/trạng thái thực xem gói hiện hành của từng phase và [sổ công việc](#work-state).
...
| [R05 — 5 hiện hành, 2 ID Future](#r05) | Coding rules C++/Web, môi trường và đặc tả test kỹ thuật | Mẫu đúng/sai, build nhỏ và toolchain theo quyền; chưa thay pilot thật |
| [R06 — 9 hiện hành, 2 ID Future](#r06) | Ba bản đồ, adapter, mapping hai chiều, change/impact/resume sâu | Thiếu/sai map, event/data/config, không diff, chu kỳ và nguồn đổi giữa lượt |
...
| [R09 — 12 hiện hành, 2 ID Future](#r09) | Workshop thật: backend–web, đổi giữa MVP, event/admin, release lab và thay đổi sau release | Đi đầu–cuối và thử reject, gián đoạn, hotfix, lỗi một phần, tương thích, khôi phục; dữ liệu giả |
...
R01 đã APPROVED và cho mở R02 trong phạm vi được phép. Phase sau giữ gate phase trước, R09 giữ quyền/gate pilot riêng. Task phụ thuộc hợp đồng, nguồn, công cụ/quyền và kết quả thực sự cần, không mặc định N chờ toàn N−1. Nêu prerequisite tại task trước mở; vẫn một việc triển khai, không bypass gate sản phẩm hoặc dùng đầu vào chưa đủ. Test helper khi lát cắt làm được; AI theo luồng và gói quyền/quota rõ, không dồn mọi kiểm chứng đến R09.
...
| R02-T06 — Ghi có căn cứ | [Một lượt hợp tác/Git-first](KIDEA_DESIGN.md#lean-operation-approved): đúng root/đích/quyền, bản trước/review và dấu bận/pending. | Đủ/thiếu căn cứ Git/snapshot, CREATE-no-overwrite, lỗi quyền/I/O/ngắt/readback/cleanup. Không tự replay/restore sau ngắt; không còn proof Win32 chống editor. Giữ kết quả/protocol cũ đúng bản, kiểm writer mới riêng. |
...
Các caller ghi T07/T08/T09 áp dụng [mô hình một lượt hợp tác](KIDEA_DESIGN.md#lean-operation-approved), [KA-10](KIDEA_ACCEPTANCE.md#acceptance-write-model) và [KQ-02](KIDEA_QUALITY.md#write-model-cooperative-approved). T10/T11/R10 kiểm cùng mô hình: Git-first đúng byte/retention, root/quyền/CREATE, dấu bận/pending/readback, không tự replay/restore sau ngắt. Không còn bảo đảm Win32 ngăn writer ngoài luồng; giữ giới hạn và evidence lịch sử.

<a id="r02-subtasks"></a>

### Phân rã R02 — kế hoạch theo từng lát cắt

Mỗi ô chỉ rõ đầu ra/kiểm tra và prerequisite thực. [H] cần xác nhận đúng gói trước phần phụ thuộc; [A] làm trong hợp đồng/quyền đã có. Không áp S01→S02→S03 hoặc task trước→sau chỉ vì số thứ tự, nhưng không dùng điều này để vượt đầu vào, quyết định hay gate thật. Phần đã thực hiện dưới đây giữ lịch sử; chỉ tạo subtask mới khi có lát cắt/đầu ra độc lập cần quản lý, không để phân rã lịch sử buộc tăng thủ tục.
...
| R02-T06 | **R02-T06-S01 [H]** Lịch sử r1/r2 | **R02-T06-S02 [A]** Lịch sử worker native và test đúng bản | **R02-T06-S03 [A]** Lịch sử 3 phiên AI/status, quota dùng hết | **R02-T06-S04 [A]** Theo hướng tinh gọn đã duyệt: hợp đồng/caller/writer hợp tác, Git-first và hồi quy xác định dữ liệu giả; không AI/ACL/VM |
...
|---|---|---|---|
| **T06 — ghi có căn cứ** | [Mô hình hợp tác/Git-first đã duyệt](KIDEA_DESIGN.md#lean-operation-approved); giữ root/quyền/CREATE, pending và không tự replay/restore. | S04 thay primitive/caller và kiểm thử xác định trên dữ liệu giả. S02/S03 native giữ lịch sử, không chứng nhận writer mới. | Đúng raw byte/retention, dấu bận, ngắt/readback/cleanup; giữ mọi FAIL cũ, không chạy protocol cũ trên nguồn mới hoặc mở quota AI. |
...
**Kết quả hiện hành:** [Human đã nghiệm thu](docs/R05_ACCEPTANCE.md) gói backend/Web tại `98fb341`; R05 và R05-T07-S02 **DONE đúng phạm vi hiện hành**. T01/T02/T03/T06/T07 được khép; T04/T05 Future chưa roadmap, không DONE/PASS. Các trạng thái chờ nghiệm thu/chưa build/chưa phân rã phía dưới giữ lịch sử, không thay biên bản mới.
...
| R05-T02 — C++ / Ubuntu | Profile C++20, tài nguyên/concurrency, build/test và cấu hình đích; Docker local Ubuntu userspace, cloud qua SSH/Docker khi có máy/quyền. | Mẫu đúng/sai; toolchain/image/kiến trúc thực; không mặc định -march=native hoặc thêm VM riêng; benchmark đúng máy đích, không lấy emulation/local làm hiệu năng cloud. |
...
| R05-T04 — Android | **FUTURE — chưa roadmap**; ID giữ để truy kế hoạch/bằng chứng cũ, không là công việc xếp vào phase hiện hành. | Không chặn nghiệm thu phạm vi backend/Web; chưa DONE/PASS. Mở lại chỉ khi Human quyết định phạm vi/kế hoạch mới. |
| R05-T05 — iOS | **FUTURE — chưa roadmap**; ID giữ để truy kế hoạch/bằng chứng cũ, không là công việc xếp vào phase hiện hành. | Không chặn nghiệm thu phạm vi backend/Web; chưa DONE/PASS. Mở lại chỉ khi Human quyết định phạm vi/kế hoạch mới. |
...
| R06-T04 — Quan hệ triển khai Android | **FUTURE — chưa roadmap**; ID giữ để truy kế hoạch/bằng chứng cũ, không là công việc xếp vào phase hiện hành. | Không chặn nghiệm thu phạm vi backend/Web; chưa DONE/PASS. Mở lại chỉ khi Human quyết định phạm vi/kế hoạch mới. |
| R06-T05 — Quan hệ triển khai iOS | **FUTURE — chưa roadmap**; ID giữ để truy kế hoạch/bằng chứng cũ, không là công việc xếp vào phase hiện hành. | Không chặn nghiệm thu phạm vi backend/Web; chưa DONE/PASS. Mở lại chỉ khi Human quyết định phạm vi/kế hoạch mới. |
...
**Gói hiện hành:** [R07 r1](proposals/r07-offline-view-r1.md) APPROVED D1–D5; [kết quả triển khai/kiểm](tests/evidence/r07/implementation-r1.md) có nguồn cuối và raw FAIL/PASS. T01–T05 DONE trong phạm vi [nghiệm thu Human](docs/R07_ACCEPTANCE.md) tại `94dc339`. [Chỉ Chrome](docs/R07_CHROME_SCOPE.md); Safari ngoài phạm vi. [Chuyển gate pilot sang R09-T14](proposals/r07-pilot-gate-r1.md) đã APPROVED; lượt pilot vẫn NOT_RUN và bắt buộc trước khép R09. R08 r1 đã được duyệt D1–D3; lab R2 vẫn có gate riêng.
...
**Gói hiện hành:** [R08 r1](proposals/r08-delivery-r1.md) APPROVED D1–D3; hướng dẫn và14tình huống review, R2-A15vector, [B1 build sạch](tests/evidence/r08/product-build-execution-r1.md), [B2 hai target](tests/evidence/r08/b2-execution-r1.md), [G2 local cuối](tests/evidence/r08/final-r1.md) và [cloud độc lập](tests/evidence/r08/cloud-execution-r1.md) đã đạt đúng phạm vi. Core283, C++48, Web32+4, Chromium18,6mutation DETECTED; giữ mọi FAIL. Cloud đã xóa workload VM/disk thật, observer cảnh báo/giữ backup, VM mới restore đúng56record/bảng và job hồi phục;7loại tài nguyên lab đọc lại đều rỗng. Nguồn/pilot/bằng chứng cũ bảo toàn,137container R08 local dừng. Ngoại lệ B1 đã dùng xong; mọi manifest thực thi đã tiêu thụ, không chạy lại cùng output/deadline. **R08-T01–T06 và phase R08 DONE theo [nghiệm thu Human](docs/R08_ACCEPTANCE.md)** ngày2026-09-17 cho gói tại `16b2223`. Bước tiếp: chuẩn bị gói R09; chưa mở thực thi pilot hoặc R10. R09 pilot thật/R09-T14 vẫn NOT_RUN; sau R08 còn hai phase R09/R10. R05 DONE backend/Web; Android/iOS Future chưa roadmap, Apple Silicon NOT_RUN.

| Task | Đầu ra hữu hạn | Kiểm chứng bắt buộc |
|---|---|---|
| R08-T01 — Phân rã kế hoạch sản phẩm | Phase/task/subtask nhỏ, dependency/output/test/gate; chuẩn bị gói review theo cách phối hợp hiện hành, giữ gate sản phẩm riêng. | Duyệt plan khác bắt đầu code; bao phủ backend/Web/ops/admin/SEO trong phạm vi hiện hành. |
...
| R08-T03 — Build/kiểm chứng và môi trường sớm | Bộ lệnh build/test dùng lại, nguồn sạch/công cụ/config/version rõ; điều khiển từ local, chưa cần dịch vụ CI. Backend Docker local Ubuntu userspace; test nặng/performance cloud qua SSH/Docker khi Human cấp host/quyền, không remote orchestrator. Một điểm vào deploy gọi script phụ, dùng chung logic cùng cơ chế và tách config DEV/PROD. | Kiểm đúng artifact/config/kiến trúc/máy đích, browser Web theo ma trận hiện hành và đọc lại kết quả; không dùng local/emulation thay benchmark máy chủ hoặc cùng-host volume/observer thay backup/quan sát độc lập. Lab kín không mở index; không đợi code xong mới chuẩn bị deploy hoặc tự xây thư viện template cho mọi nền. |
...
**Trạng thái 2026-09-20:** lát backend readiness/lifecycle của T05 đạt component PASS trên Pilot source `4ee234e`; recovery protocol v3 đã xử lý giới hạn request mà không tăng cap. `R09-T04-OUTPUT-r1` revision 13 đã được Human duyệt đúng digest trình duyệt, public APPROVE xác minh byte và public READ đọc lại `APPROVED`. `W-009-ADMIN-OPS` vẫn IN_PROGRESS, không blocker và còn `IMPACT_REVIEW_REQUIRED`. Kết quả chưa chứng minh readiness production, M3/M7, WQ, restore/release, G2 hoặc nghiệm thu T05/R09. Tiếp theo là coverage reconciliation, WQ/vận hành hữu hạn và semantic impact refresh; R10 chưa mở. [Chi tiết và giới hạn](docs/R09_T05_PROGRESS.md#hiện-hành--review-r13-đã-được-human-duyệt).
...
| R09-T03 — Web lát cắt đầu tiên | Intro/list/detail/đăng ký theo task pilot; SSR/prerender và UI tối thiểu. | Test thật, map/assertion; giữ checkpoint MVP còn thiếu admin/event cho task kế. |
...
| R09-T06 — Android pilot | **FUTURE — chưa roadmap**; ID giữ để truy kế hoạch/bằng chứng cũ, không là công việc xếp vào phase hiện hành. | Không chặn nghiệm thu phạm vi backend/Web; chưa DONE/PASS. Mở lại chỉ khi Human quyết định phạm vi/kế hoạch mới. |
| R09-T07 — iOS pilot | **FUTURE — chưa roadmap**; ID giữ để truy kế hoạch/bằng chứng cũ, không là công việc xếp vào phase hiện hành. | Không chặn nghiệm thu phạm vi backend/Web; chưa DONE/PASS. Mở lại chỉ khi Human quyết định phạm vi/kế hoạch mới. |
...
| R09-T12 — Ngắt phiên và pending side effect | Gián đoạn nhiều cấp/thiếu công cụ, thao tác đã/chưa xảy ra; mất kết nối sau gửi lệnh. | Đối chiếu thực tế trước tiếp tục; không sửa hộ trạng thái để PASS. Job/cảnh báo độc lập phiên AI/laptop, không lặp migration. Ghi local theo một lượt hợp tác, giữ file/bản trước/điểm tiếp tục; không tự replay/restore sau ngắt, phương án xử lý cần đúng quyền/căn cứ. |
...
| R09-T14 — Khép pilot và sửa lỗi Kidea | Tổng hợp toàn bộ bằng chứng, lỗi, impact và hồi quy. | Mỗi sửa lỗi tách subtask có phạm vi; Human quyết định quay phase nào, không mở refactor vô hạn. Tổng hợp luồng release/đọc lại/gián đoạn/phục hồi, không thay bằng mẫu hồ sơ. **Bắt buộc dựng/đối chiếu và đo view Chrome trên hồ sơ pilot thật hợp lệ**, tiếp nhận R07-T04/V12 theo [nghiệm thu R07](docs/R07_ACCEPTANCE.md); hiện NOT_RUN, không khép R09 khi thiếu. Giữ ngưỡng/phạm vi đã chốt, không tự tạo metadata giả. |
...
| R10-T03 — Nâng cấp và rollback lỗi | Đường nâng cấp được hỗ trợ, lỗi giữa chừng/khôi phục; ghi theo [mô hình hợp tác](KIDEA_DESIGN.md#lean-operation-approved). | Bảo toàn căn cứ, nhận diện dở và kiểm tra xử lý được phép; không tự replay/restore sau ngắt. Không còn proof Win32 chống editor; giữ lỗi quyền/CREATE/pending/cleanup và giới hạn không chống hỏng ổ/mất điện. |
...
| Ghi/resume/Git/pending side effect, KA-10–13; KQ-02/03; [mô hình hợp tác/Git-first](KIDEA_DESIGN.md#lean-operation-approved) | R02/R06 | R09-T12/T13, R10-T03/T04; đúng bản trước/review, dấu bận/pending, không tự replay/restore; giữ quyền/gate |
...
| [Checkpoint/resume Git](KIDEA_DESIGN.md#git-checkpoint), ghi theo [mô hình hợp tác](KIDEA_DESIGN.md#lean-operation-approved) | R02-T04/T06/T07/T08/T09; R06-T10 | KA-10–13; R02-T10/T11; R09-T12/T13; R10-T03/T04 |
