...
Thử cả quyền đủ thì tiếp tục và quyền thiếu thì dừng. Mô hình lỗi ghi gồm lỗi quyền, ngắt tiến trình và sửa ngoài luồng; không nhận chống hỏng ổ đĩa/mất điện vật lý. Phạm vi bảo đảm ghi được cập nhật đúng hai ngoại lệ r2 dưới đây, không bỏ nghĩa vụ thử thông thường. Approval R01 này chưa chốt fixture/schema, số lần chạy, ngưỡng hoặc toàn bộ KQ-01–03 cho các nhóm case khác; các phương pháp đo còn đề xuất bên dưới vẫn phải qua gate tương ứng. Không coi approval là case đã thực thi.

<a id="write-model-r2-approved"></a>

**Giới hạn ghi bản đầu đã được cập nhật:** Human “Ok nhé” sau [answer 5466a42](https://github.com/Kynderis/kidea/blob/5466a4230e5edafe243aca0bad2d0a95e1596f48/answer.md) duyệt [R02-T06-S01-r2](KIDEA_DESIGN.md#safe-write-namespace-review). Đối với KQ-02 và cam kết an toàn ghi của nhóm điều phối/lưu–tiếp tục, chỉ loại khỏi bảo đảm hai can thiệp đồng thời: tiến trình khác tạo tên hard link mới cho file hoặc đổi hướng thư mục **ngay trong lượt ghi**, dù vô tình hay cố ý. Khi đó có thể ảnh hưởng cả đường dẫn ngoài danh sách, không bảo đảm phát hiện kịp hoặc khôi phục được. Đây là thay đổi phạm vi bảo đảm r1 được Human duyệt, không tự hạ chuẩn theo kết quả.

Không cấp quyền chủ động ghi ngoài đích, bỏ kiểm tra quyền/identity hoặc tiếp tục khi phát hiện vi phạm. Hard link nhiều tên/reparse đã có hoặc đích không xác minh được vẫn phải bị từ chối; phát hiện can thiệp trong lượt phải dừng phần còn lại, giữ bằng chứng/chưa xác nhận và không tự restore khi thiếu căn cứ. Giữ đầy đủ thử sửa/lưu/xóa/đổi tên thông thường, lỗi quyền, ngắt tiến trình, CREATE, phục hồi và cleanup. Các FAIL/counterexample r1 giữ nguyên, không đổi thành PASS/N/A hoặc cộng thành nghiệm thu r2. Không đổi ngưỡng, số phiên AI, gate hay quyền khác.
...
- Mất dữ liệu hoặc tác dụng ghi ngoài danh sách trong [mô hình được bảo đảm](#write-model-r2-approved) vẫn chặn nghiệm thu. Vượt gate, chủ động ghi ngoài quyền, replay nguy hiểm hoặc báo đạt sai không được miễn bởi giới hạn mô hình. Hậu quả quan sát ở hai can thiệp liên kết ngoài bảo đảm vẫn phải báo đúng và giữ bằng chứng, không nhận là an toàn đạt. Với lỗi trong nghĩa vụ đã nhận, sửa nguyên nhân, rà ảnh hưởng và chạy lại nhóm liên quan; không chọn lần chạy đẹp nhất.
...
| KQ-02 — An toàn ghi | Trong [mô hình hỗ trợ r2](#write-model-r2-approved): 0 mất nội dung đã lưu hợp lệ trước thao tác; 0 thay đổi file ngoài danh sách được phép. Sau lỗi, hoặc bản cũ nguyên vẹn, hoặc trạng thái dở được nhận diện và khôi phục được theo hợp đồng; không giả thành công hoặc tự restore thiếu căn cứ. Không bảo đảm an toàn/khôi phục cho đúng hai can thiệp liên kết đồng thời đã loại trừ | KA-02/03/10/13/26/29: so nội dung/hash trước/sau; thử lỗi quyền ghi, ngắt tiến trình trước ghi/giữa chuỗi cập nhật/sau ghi trước xác nhận và nguồn bị sửa ngoài luồng thông thường. Phải có bản/căn cứ phục hồi phần bị tác động trong mô hình hỗ trợ; không yêu cầu mọi file cập nhật nguyên tử cùng lúc. Giữ phép từ chối liên kết/đích sai đã có, không đổi counterexample r1 thành PASS/N/A |
...
KQ-02 áp dụng lỗi ghi được tiêm vào fixture, dừng tiến trình và thay đổi file ngoài luồng thông thường trong [giới hạn r2 được duyệt](#write-model-r2-approved). Không nhận đã chứng minh chống hỏng ổ đĩa/mất điện vật lý chỉ từ các thử này; chính sách backup/khôi phục và giới hạn hệ thống file phải được nêu ở R02/R10. Caller ghi T07/T08/T09 và kiểm chứng T10/T11/R10 phải dùng/công bố cùng mô hình; không dùng ngoại lệ hard link mới/đổi hướng thư mục đồng thời để bỏ các thử an toàn ghi còn lại, đổi quyền hoặc nhận kết quả chưa có.
