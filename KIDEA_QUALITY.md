...
Thử cả quyền đủ thì tiếp tục và quyền thiếu thì dừng. Mô hình ghi hiện hành là một lượt hợp tác, thử lỗi quyền/I/O/ngắt tiến trình, dấu bận, CREATE và retention; không bảo đảm chống writer ngoài luồng hoặc hỏng ổ/mất điện. Phạm vi mới được Human duyệt ngày 2026-09-14 dưới đây; approval không là kết quả test, không tự chọn số lần/ngưỡng hoặc toàn KQ-01–03.

<a id="write-model-r2-approved"></a>
<a id="write-model-cooperative-approved"></a>

**Mô hình hiện hành — một lượt hợp tác/Git-first:** Human **“Tôi đồng ý hướng bạn đề xuất”** trong hội thoại ngày 2026-09-14 duyệt [hợp đồng tinh gọn](KIDEA_DESIGN.md#lean-operation-approved), thay mô hình Win32 r1/r2. Một lượt Kidea sửa cùng project qua cửa sổ/máy; công cụ sửa nguồn tuần tự, không có writer/editor/sync ngoài luồng. Dấu bận không là khóa phân tán hoặc rào chống phần mềm khác. Bỏ nghĩa vụ chứng minh Win32 ngăn editor sửa/xóa/đổi tên; giữ đúng đích/quyền/bản trước, pending/readback và không tự replay/restore sau gián đoạn.

Không cấp quyền chủ động ghi ngoài đích, bỏ root/path/identity hoặc CREATE-no-overwrite. Đường liên kết không hỗ trợ/đích không xác minh được bị từ chối; phát hiện lệch thì dừng và giữ chưa xác nhận. Git-first phải đọc đúng raw byte, ref giữ lại và thời hạn; thiếu hoặc khác byte dùng snapshot tối thiểu. Lịch sử review không bị cleanup cùng bản phục hồi tạm. [R1/r2](KIDEA_DESIGN.md#safe-write-namespace-review) cùng mọi lỗi/protocol/manifest cũ giữ lịch sử, không đổi PASS/FAIL hoặc dùng chúng chứng nhận writer mới. Không đổi G2/gate/quota/quyền môi trường.
...
- Mất nội dung cần giữ hoặc tác dụng ghi ngoài danh sách trong [mô hình hợp tác](#write-model-cooperative-approved) vẫn chặn nghiệm thu. Vượt gate/quyền, replay nguy hiểm hoặc báo đạt sai không được miễn. Ngoài mô hình, hậu quả quan sát vẫn phải báo đúng, không gọi là an toàn đạt. Lỗi thuộc nghĩa vụ đã nhận phải sửa/đánh giá ảnh hưởng và hồi quy; không chọn lần đẹp.
...
| KQ-02 — An toàn ghi | Trong [mô hình hợp tác](#write-model-cooperative-approved): 0 mất nội dung đã lưu hợp lệ cần giữ, 0 thay đổi ngoài danh sách được phép. Sau lỗi có bản/căn cứ đúng byte và nhận diện dở, không giả thành công hoặc tự replay/restore sau ngắt. Không cam kết chống editor/writer ngoài luồng | KA-02/03/10/13/26/29: đủ/thiếu quyền, dấu bận, Git đúng/sai byte/ref/khả năng truy lại, snapshot tối thiểu, CREATE, lỗi I/O/ngắt trước–giữa–sau ghi/pending/readback/cleanup. Review retention riêng recovery. Không yêu cầu Win32 proof hoặc nhiều file đổi nguyên khối; không đổi evidence cũ thành PASS mới |
...
KQ-02 dùng lỗi ghi/ngắt tiến trình được tiêm trên dữ liệu tổng hợp và mô hình hợp tác đã chốt. Không nhận chống hỏng ổ/mất điện hoặc ngăn mọi thay đổi ngoài luồng. Caller T07/T08/T09 và kiểm chứng T10/T11/R10 phải dùng/công bố cùng mô hình, giữ root/quyền/CREATE/bản trước/pending/cleanup; không dùng approval mới để che kết quả chưa có hoặc thay protocol cũ.
