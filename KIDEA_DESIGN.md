...
#### Cơ chế ghi/phục hồi local — r1 đã duyệt, phạm vi D1 được cập nhật bởi r2
...
T05 đã thử bộ đọc và ba phiên AI theo [bằng chứng](tests/evidence/r02-t05.md); tại thời điểm T05 chưa có chức năng ghi. T06 làm cho các chức năng sau này ghi đúng đích và nhận ra việc dở; giữ nguyên [bản trước/cách phục hồi](#interrupted-write-contract), [schema 2](#schema-v2-contract) và quyền G4. **Căn cứ hiện hành là cơ chế r1 cùng giới hạn D1-r2 đã được duyệt dưới đây**; [kết quả triển khai/kiểm chứng nội bộ](tests/evidence/r02-t06.md), trạng thái ở sổ công việc. Giữ xác nhận/bản trình r1 làm lịch sử, không suy approval hoặc một probe thành kết quả ghi an toàn đã đạt.

**D1 — Khóa bằng Windows trong lượt ghi, trong phạm vi D1-r2.** Một worker PowerShell/.NET local do Node gọi dùng cơ chế mở file của Windows để giữ quyền ghi độc quyền lên đích và ngăn sửa/xóa/đổi tên thông thường đối với các đường dẫn cần bảo vệ. Không chỉ đặt cờ “đang bận”. Lượt ghi thứ hai không lấy được khóa thì dừng; không xếp hàng chờ vô hạn hoặc tự phá khóa. Giữ khóa qua kiểm tra byte trước, ghi, đọc lại và xử lý lỗi; đầu vào chi phối cũng phải được giữ ổn định trong [mô hình hỗ trợ r2](#safe-write-namespace-review). Việc lấy khóa thất bại vì editor/tiến trình khác đang dùng file là kết quả cần báo, không được tắt bảo vệ để ghi cho được.

Lợi ích: Windows có thể từ chối thao tác xung đột, thay vì chỉ hy vọng phần mềm khác đọc cờ của Kidea. Đánh đổi: thêm lớp code Windows và có thể tạm chặn lưu/đọc ở editor khi file đang bị khóa. Chọn Windows/ổ NTFS local cho lát cắt ghi đầu tiên; chưa hỗ trợ ổ mạng, thư mục đồng bộ đang hoạt động, reparse point/junction/symlink, hard link nhiều tên hoặc đích không xác minh được. Không tự chuyển project hoặc tắt phần mềm đồng bộ. Đường chưa hỗ trợ phải báo rõ và không ghi; đây không là cắt nghĩa vụ hỗ trợ lâu dài hoặc thay giới hạn status chỉ đọc.

Nguồn kỹ thuật: [FileShare](https://learn.microsoft.com/en-us/dotnet/api/system.io.fileshare?view=net-10.0) quy định quyền chia sẻ khi mở file; [CreateFileW](https://learn.microsoft.com/en-us/windows/win32/api/fileapi/nf-fileapi-createfilew) nêu sharing violation, quyền delete liên quan rename và mở thư mục. Đây là cơ sở thiết kế, **chưa là bằng chứng worker đã chống được mọi race**. Không dùng riêng FileStream.Lock theo vùng byte hoặc kiểm tra hash rồi đóng file để nhận quyền độc quyền. Worker cần xác minh handle/đường dẫn thật và bảo vệ các thư mục cha khỏi đổi tên/xóa thông thường; thiếu bảo đảm trong mô hình hỗ trợ r2 thì chưa mở đường ghi. R2 không bảo đảm ngăn tạo hard link mới hoặc đổi hướng thư mục đồng thời, không diễn giải share-mode/oplock là đã bảo vệ hai trường hợp đó. Không dùng quyền administrator/backup để vượt ACL.

Tại lúc trình đã đọc thấy host Windows build 26200, PowerShell 7.6.5 hiện có trong runtime Codex và ổ D: là Fixed/NTFS. D1 chọn dùng PowerShell/.NET có sẵn, có thể gọi Win32 từ worker, không cài thêm package/runtime/global hoặc đổi Node 24. Trước code phải xác minh lại runtime/path/hash; không có thì báo thiếu, không tự tải thay thế. Không sửa skill để cho AI chạy lệnh ghi công khai trong lát cắt này; worker là phần nội bộ được thử trên bản sao trước tích hợp init/approve/resume.

**D2 — Ghi qua handle đang giữ khóa; nhận và xử lý bản dở thay vì hứa nhiều file đổi cùng lúc.** Chuẩn bị nội dung/bản trước và hồ sơ lượt ghi trước lần thay đầu tiên; đọc lại đủ căn cứ. UPDATE ghi qua chính handle độc quyền đã đối chiếu, không thả khóa rồi mở lại/rename đè. CREATE dùng tạo mới độc quyền, nếu đích đã xuất hiện thì dừng, không chuyển sang UPDATE; file rỗng đã tạo nhưng chưa ghi xong là tác dụng đã xảy ra cần giữ. Đọc lại/flush trong cùng quyền khóa; chỉ ghi nhận thành công khi đủ mọi đích và kiểm tra liên quan. Cách này có thể để lại file ghi một phần khi tiến trình bị ngắt, đổi lại không mở khoảng thả khóa để thay file theo tên.

Trong cùng tiến trình, chỉ tự phục hồi bước UPDATE lỗi của chính Kidea khi khóa bảo vệ được giữ liên tục, có đúng bản ngay trước, đầu vào/quyền còn đúng và đủ căn cứ không có sửa chen. Phục hồi cũng ghi/đọc lại dưới khóa; lỗi phục hồi giữ UNKNOWN/OTHER và bằng chứng, không báo an toàn. Khi tiến trình đã ngắt/mất khóa, đối chiếu BEFORE/PLANNED/OTHER theo T04; không lấy riêng hash khớp làm bằng chứng không có sửa chen để tự hoàn tác. Chưa rõ nguồn gốc thì giữ và hỏi. CREATE dở không tự bị xóa theo quyền restore UPDATE hoặc quyền dọn payload tạm.

Lượt đang ghi hoặc chưa đối chiếu xong phải làm status/khởi động chức năng ghi báo chưa đủ căn cứ, không trả tiến độ đã xác minh. Cần dấu lượt ghi tại `.kidea/checkpoints/` tồn tại trước sửa nguồn và phát hiện được dù work.checkpointRef chưa cập nhật; chỉ đọc vùng lượt ghi đã định nghĩa, không quét/nạp mọi file làm trạng thái. Dấu này hỗ trợ phát hiện, không thay khóa hệ điều hành hoặc thành tracker thứ hai. Giữ hồ sơ chuẩn bị/bằng chứng quan sát đã có theo bản riêng để record đang cập nhật hỏng không xóa căn cứ; chỉ đóng lượt sau đọc lại đầy đủ. Khóa còn dấu nhưng không còn tiến trình không tự là giấy phép dọn hoặc replay. Dọn payload vẫn theo T04; không xóa lịch sử review/Git/nguồn mới tạo.

Mô hình lỗi cần kiểm chứng theo r2: tiến trình bị dừng ở từng ranh giới, lỗi I/O/quyền, ghi/phục hồi dở; hai writer cạnh tranh; editor/tiến trình khác mở sửa/xóa/đổi tên thông thường đối với đích hoặc thư mục cha trước và trong khóa; nguồn đổi trước khóa, sau mất khóa; CREATE bị chiếm tên; từ chối hard link nhiều tên/reparse/đường thoát root đã có hoặc phát hiện được; checkpoint hỏng/mất con trỏ; status chạy giữa lượt; lỗi cleanup. Chỉ hai can thiệp **tạo tên hard link mới hoặc đổi hướng thư mục ngay trong lượt ghi** nằm ngoài bảo đảm an toàn bản đầu, kể cả xảy ra vô tình; r2 không hứa phát hiện kịp hoặc phục hồi được hậu quả của chúng. Dùng tiến trình thứ hai thật trên dữ liệu tổng hợp để kiểm tra xung đột trong mô hình hỗ trợ, không chỉ mock hash; giữ các counterexample r1 đúng kết quả lịch sử. D1/D2 chưa chứng nhận chống mất điện/hỏng ổ/ghi raw disk, administrator/kernel can thiệp, mọi biến thể filesystem hay nhiều file đổi nguyên khối. Không dùng giới hạn này để chủ động ghi ngoài quyền hoặc cho phép đè sửa chen trong mô hình đã nhận.
...
Chi tiết primitive S02 đã kiểm chứng nằm tại [write-internal.mjs](.agents/skills/kidea/scripts/write-internal.mjs) và worker Windows đi kèm: caller tin cậy chuẩn bị quyền/đồ thị hiện tại + dự kiến; worker giữ và đối chiếu hợp các byte đầu vào/đích rồi mới ghi; Node xác nhận proof đúng kế hoạch trước FINALIZE trong cùng lượt khóa. `COMPLETED_BYTES` chỉ nói byte/quan hệ đã đối chiếu, không ghi DONE/APPROVED cho công việc. `writer.lock` rỗng là tên ổn định cho khóa handle hệ điều hành, không dùng file có/không có làm quyền chiếm khóa; pending vẫn chỉ phục vụ discovery.

Cleanup là thao tác riêng có quyền/điều kiện cụ thể, không mặc định chạy khi ghi xong. Chỉ dọn payload trước/dự kiến đúng operation sở hữu; đối chiếu hoàn tất LEAF/GROUP, gate, VERIFY, retention và các checkpoint còn được Ref trực tiếp giữ. Ref lịch sử có VersionRef không tự thành trạng thái mới. Xóa theo handle, xác minh vắng mặt rồi cập nhật receipt; lỗi/ngắt giữ pending, không tự dựng lại/xóa file chen vào. Request/prepared/observations và bản chứng cứ cần lưu vẫn giữ, nên cleanup không có nghĩa xóa sạch mọi byte audit.

Giới hạn triển khai hiện tại: primitive nhận project đã hợp lệ và thư mục cha đích đã tồn tại; dùng SNAPSHOT local, từ chối GIT/EXTERNAL/căn cứ vắng mặt cho đường ghi trước tác dụng lên đích. Không giảm khả năng đọc GIT của status. Init mới, caller xác thực approval/permission, reconciliation sau restart và tích hợp init/approve/resume còn thuộc công việc tiếp theo; không suy một primitive thành lõi/caller hoàn chỉnh. Giới hạn không cho bỏ dependency hoặc che đường chưa hỗ trợ bằng mặc định.
...
#### R02-T06-S01-r2 — giới hạn bảo vệ đường dẫn bản đầu, đã duyệt

Human **“Ok nhé”** sau [answer 5466a42](https://github.com/Kynderis/kidea/blob/5466a4230e5edafe243aca0bad2d0a95e1596f48/answer.md) đã duyệt đúng gói R02-T06-S01-r2 và phần giải thích hệ quả trong answer đó. [Bản trình r2](https://github.com/Kynderis/kidea/blob/5466a4230e5edafe243aca0bad2d0a95e1596f48/KIDEA_DESIGN.md#safe-write-namespace-review) giữ làm căn cứ lịch sử. R2 **thay phạm vi bảo đảm D1-r1 chỉ đối với hai can thiệp liên kết đồng thời nêu dưới**, không thay D2 hoặc các bảo vệ còn lại. Tiêu chí/caller được đồng bộ theo giới hạn đã duyệt; S02 có [bằng chứng nội bộ trên dữ liệu giả](tests/evidence/r02-t06.md), chưa có chức năng ghi công khai. Trạng thái duy nhất ở [sổ công việc](KIDEA_ROADMAP.md#work-state); approval này độc lập với approval cách phối hợp.

**Vấn đề có bằng chứng khi trình r2:** [probe share-mode](tests/r02-t06/native-lock-evidence.md) có 7 PASS/4 FAIL; file đang giữ vẫn có thể nhận hard link mới và thư mục rỗng đang giữ vẫn có thể đổi thành junction. Probe bổ sung RWH/RH oplock cũng ghi nhận các thao tác này thành công trước khi owner thả handle, gồm handle chỉ ghi thuộc tính mở trước/sau khi cấp oplock; xem [bằng chứng T06](tests/evidence/r02-t06.md). Đây là các phản ví dụ trên tổ hợp đã thử, không chứng minh mọi cơ chế Windows đều bất khả thi. Tại thời điểm trình gói chưa có worker ghi dữ liệu thật hoặc lỗi mất dữ liệu project; không dùng đoạn bằng chứng lịch sử này thay trạng thái triển khai ở roadmap.

**D1-r2 — Giới hạn bản đầu vào môi trường không đổi liên kết trong lượt ghi.** Kidea vẫn phải chặn các thao tác sửa/lưu/xóa/đổi tên thông thường trong mô hình được kiểm chứng, giữ khóa đích/nguồn và bảo vệ các đường cha cần thiết. Bản đầu **không bảo đảm an toàn nếu tiến trình khác tạo tên hard link mới cùng trỏ tới file hoặc đổi hướng thư mục ngay trong lúc ghi**, dù xảy ra vô tình hay cố ý. Điều kiện này phải được nêu rõ trong phạm vi sử dụng, không dùng câu “thư mục tin cậy” thay giải thích, không nhận một lần quét là chứng minh không có can thiệp tương lai.
...
- Giữ lỗi/counterexample đúng bản. Không đổi FAIL của r1 thành PASS/N/A. Bộ kiểm chứng r2 phải phân biệt rõ điều kiện hỗ trợ với can thiệp ngoài bảo đảm; trong mô hình được hỗ trợ, mọi lỗi mất nội dung/ghi ngoài quyền/báo thành công sai vẫn chặn nghiệm thu. Ngoài mô hình, hậu quả đã quan sát vẫn phải báo đúng, không biến việc bỏ bảo đảm thành bằng chứng an toàn đạt.

**Phạm vi áp dụng đã đồng bộ:** D1/mô hình lỗi T06; [KA-10](KIDEA_ACCEPTANCE.md#acceptance-write-model) và [KQ-02](KIDEA_QUALITY.md#write-model-r2-approved), gồm phần điều phối/quyền. Caller ghi ở T07/T08/T09 phải dùng cùng giới hạn và điều kiện dừng; T10/T11/R10 kiểm chứng/công bố đúng mô hình, không suy kết quả r1 thành PASS r2. R2 chỉ giới hạn hai điều kiện can thiệp liên kết trong lượt nói trên, không miễn thử sửa file ngoài luồng thông thường, lỗi quyền, ngắt tiến trình, CREATE, phục hồi hoặc dọn payload. Không thay schema 2, approval sản phẩm, ngưỡng, số phiên AI hay quyền Git/pilot/deploy; không cấp quyền cài đặt/cách ly/chuyển project hoặc chủ động ghi ngoài danh sách đích.
