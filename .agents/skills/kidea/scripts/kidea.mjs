...
if (!runtimeInfo().compatibleVersion) {
  ...
  console.log(JSON.stringify({ stage: 'R09 scoped work, delivery and recovery metadata candidate', actions, implemented: ['status','init','approve','resume','change','visualize'], help: 'status is read-only; init creates initial records; approve manages scoped reviews; resume reads context, saves notes, or records scoped work transitions, reviewed work cycles, release/operation evidence and explicitly authorized metadata recovery; change manages a scoped impact plan; visualize exports a managed read-only HTML snapshot. Trusted stdin requests supply permission, never project files. No helper executes product commands or replays external effects; pending metadata completion requires a new exact grant.' }));
  ...
  try {
    ...
    const ok=['CONTEXT_READY','WAITING','NO_CURRENT_ITEM','CONTINUATION_SAVED','WORK_RECORDED','DELIVERY_RECORDED','RECOVERY_READY','RECOVERY_COMPLETED'].includes(result.state);
