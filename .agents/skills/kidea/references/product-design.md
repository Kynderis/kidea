...
Current client scope is Web. Android/iOS app support is Future with no roadmap; do not generate native profiles, request Xcode/SDK installation or make native checks a current gate. This does not change Windows/macOS Intel/Apple Silicon support for the Kidea host. Each project owns its responsive journeys, layout, breakpoints, browser/viewport matrix, touch/keyboard/accessibility requirements and acceptance evidence; preserve already approved project values rather than impose universal defaults.
...
Cover latency/capacity, availability, security/privacy, data retention, permitted data loss and recovery time, cost/resource budgets, Web performance and SEO. Do not transfer Kidea-tool targets or one pilot's numbers into another product.
...
- Propose numeric targets with rationale or identify the missing input and blocked decision. Approved targets remain NOT_RUN until tested in the required environment; do not defer numbers indefinitely or substitute a convenient platform for required project browser/target tests.
