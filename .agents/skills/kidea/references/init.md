...
Use a confirmed local filesystem, no cloud synchronization and one cooperative Kidea run. Windows and macOS Intel/Apple Silicon share the helper; actual verification is host-specific. Human does not edit during the run; serialize formatters and Git too. The busy marker prevents overlapping cooperative calls, not external edits/link changes. Network/shared/cloud-sync folders are unsupported; do not build multi-machine coordination. Multi-file creation is not atomic or power-loss durable. Stop if these facts are unknown; do not change permissions, disable sync or migrate/install anything to satisfy prechecks.
...
When local Git reads are allowed, selected source/profile bytes already stored exactly at the attached HEAD commit reachable from an existing branch/tag use a fixed commit/path reference. Dirty, untracked, non-Git, detached-HEAD or byte-different files use snapshots; no commit, stash, new Git ref, fetch, filter or hook is run to make them fit. Git discovery follows SKILL.md: trusted host configuration/absolute PATH only, not a project-selected executable; required no-lazy-fetch capability is retained. Missing optional Git falls back to snapshots; a previously recorded Git source that cannot be read requires reconciliation. Keep referenced history accessible while evidence is needed; if it disappears, stop and reconcile. This is not backup against disk loss. Review evidence and temporary pre-write copies have different retention lifetimes.

Resolve Node/helper paths as in SKILL.md. Init accepts Node.js 24+ and records actual runtime version/OS/architecture/hash; there is no pinned binary or PowerShell/C# dependency. A missing compatible runtime needs setup permission, not silent installation. Send UTF-8 JSON on stdin and close stdin; payload limit is 1 MiB and incomplete input times out after five seconds. No input-file option exists.
