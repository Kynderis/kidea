...
import { assertRuntime } from '../../.agents/skills/kidea/scripts/runtime.mjs';
...
const gitVersion=spawnSync(gitExecutable,['--no-optional-locks','--no-lazy-fetch','--version'],{encoding:'utf8',windowsHide:true,timeout:10000});
const git={executable:gitExecutable,sha256:hash(readFileSync(gitExecutable)),version:gitVersion.stdout?.trim()??null,exitCode:gitVersion.status,error:gitVersion.error?.message??null};
...
const summary={at,host:{platform:process.platform,arch:process.arch,osRelease:osRelease()},node:process.version,nodeHash:hash(readFileSync(process.execPath)),git,suites,exitCode:r.status,error:r.error?.message??null,inputsUnchanged:JSON.stringify(before)===JSON.stringify(after),before,after,stdoutHash:hash(r.stdout??''),stderrHash:hash(r.stderr??''),output:dir};
