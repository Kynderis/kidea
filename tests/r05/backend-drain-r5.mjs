...
const out=path.join(root,'tests/evidence/r05/backend-execution-r5');
const budgets=[];
const call=args=>{const reservation=acquireBudget(args);if(reservation.receipt)budgets.push(reservation.receipt);try{const r=spawnSync(docker,args,{encoding:'utf8',timeout:45000});if(r.status!==0)throw Error(r.stderr);return r.stdout;}finally{reservation.release();}};
const results=[];
...
for(const stuck of [false,true]){
 ...
 }finally{call(['stop','--timeout','5',name]);await done;call(['stop','--timeout','35','kidea-r05-e2-r3-caddy','kidea-r05-e2-r3-web','kidea-r05-e2-r3-backend']);}
}
