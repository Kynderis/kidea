...
// Pilot documentation has an approved scope amendment; review executable inputs separately.
const web=read('web-execution-r2/manifest.json');
for(const [name,hash]of Object.entries(web.sampleSource))assert.equal(sha(path.resolve(sample,'../web',name)),hash,name);
for(const stage of read('web-execution-r2/pipeline-2.json')){assert.equal(stage.exit,0);assert.equal(stage.summary.inputsUnchanged,true);assert.equal(stage.summary.timedOut,false);}
const historical=read('web-scope-r1/historical-hashes.json');
for(const [file,hash]of Object.entries(historical))assert.equal(sha(path.join(root,file)),hash,file);
...
const result={at:new Date().toISOString(),sourceHead:spawnSync('git',['rev-parse','HEAD'],{cwd:root,encoding:'utf8'}).stdout.trim(),status:'TECHNICAL_EVIDENCE_REVIEW_PASS_SCOPED',reviewer:'Current agent, evidence/static review; not a second independent reviewer or Human acceptance',sourcesVerified:Object.keys(m.sources).length,webSourcesVerified:Object.keys(web.sampleSource).length,historicalFilesVerified:Object.keys(historical).length,groups:22,checks:['final sample hashes','Web27 source hashes and final pipeline','all historical file hashes','core unchanged inputs and exit','final stage exits/counts','six mutation detections','four JUnit suites without skipped cases','unchanged Web/Caddy provenance','quota receipt'],limitations:['R4 resource incident remains a historical failure','finite synthetic vectors only','Android/iOS Future without roadmap','full application137cases remain NOT_RUN','production/performance/host-loss and Apple Silicon not certified','R05-TIDY-01 only supports historical r1 execution; no r2 waiver or later-gate reuse authorized'],r05:'IN_REVIEW_PENDING_HUMAN'};
assert.ok(process.argv[2],'new CREATE-only output path required');
