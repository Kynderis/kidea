...
Recovery r1 approved and completed on 2026-09-19: exact frozen patch applied; installed Kidea core 293/recovery 9 PASS, zero skips/cancels/todos. Fresh public READ_RECOVERY and RECOVER completed operation bef9054c using the real new Human grant and exact two-component migration pairs. Only work nextAction/checkpointRef changed; all other work data/reviews/source and original prepared request/tool identity preserved. Pending retired by helper after readback; pilot local checkpoint aba73cf, clean/no remote, review 9 DRAFT/current owner IN_PROGRESS. Source observer 8779453 and component 29/15 evidence reused; backend/Web 132/22/four presets reused. Original coverage 137 remains 11/88/38; full OP/R09 open. Routine R09 runner now includes all five new recovery safety cases. Full r1 retained seven PASS/two R06 config FAIL groups; both failed groups rerun in full with existing live-verified tools, 20 R06 + 5 boundary cases PASS. All nine groups have final PASS on unchanged source; this is not a clean full-r1 PASS. [Final regression summary](../evidence/r09/t05-observer-recovery-r1/regression-final-summary.json). [Recovery proof](../evidence/r09/t05-observer-recovery-r1/preservation.json), [next point](../../docs/R09_T05_PROGRESS.md).

History before recovery: Observer component source `8779453`: 29 Mac tests and 15 Linux Docker Chrome/TLS groups PASS on final source, with synthetic metrics and roles. No full OP promotion; original 137 coverage stays 11/88/38. Prior 132 HTTPS, 22 SQL, core 293 and four presets are reused; installed backend/Web/core are unchanged. REVISE revision 9 DRAFT/READ WAITING; author wrapper timeout 1200 s interrupted SAVE. Pending `bef9054c` remains, work BEFORE/pilot HEAD 877 unchanged. Installed recovery rejects the 83,111,093-byte request over 64 MiB. Candidate patch (128 MiB prepared request/64 MiB files/preflight/no implicit tool change) passed 293 core + 9 recovery tests; 13 B tests passed in mixed r4, with one retained invalid-fixture FAIL, corrected and final recovery 9 PASS. Candidate only read the actual pending operation as READY; no work/marker changes. Human approval of [recovery r1](../../proposals/r09-observer-checkpoint-recovery-r1.md) is needed before applying the patch and new exact code migration/RECOVER. T05/R09 remain IN_PROGRESS/DRAFT; no G2/Human acceptance. See [progress](../../docs/R09_T05_PROGRESS.md) for raw provenance, next C++ sources/W5 and remaining obligations.
