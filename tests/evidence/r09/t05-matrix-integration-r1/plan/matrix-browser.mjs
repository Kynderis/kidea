...
export async function checkMatrix({context,base,record,closeExisting}){
 ...
 const evidence={intents:[],ui:[],race:null,scope:'real TLS/backend; only lost replies and the capacity race timing are controlled'};
 const prefix='matrix-';
 ...
 async function save(p,id,patch,dialogExpected){
  ...
  await p.getByText('Lần đó đã lưu thay đổi.',{exact:true}).waitFor();await p.getByRole('button',{name:'Lưu nội dung',exact:true}).waitFor({state:'visible'});await p.locator('fieldset:not([disabled])').waitFor();
