...
for(const zoom of [1,2]){
 ...
 try{for(const width of [390,1440]){
  ...
  for(let i=0;i<6;i++){
   ...
   assert.ok(open<=5000);for(const v of [got.search,got.filter,got.detail])assert.ok(v<=200);assert.equal(got.overflow,false);assert.equal(got.width,width/zoom);assert.equal(got.dpr,zoom);assert.equal(got.items,10);assert.ok(got.body.includes('W-001'));assert.ok(got.body.includes('UNEXPANDED'));assert.ok(got.body.includes('UNKNOWN'));
  }
