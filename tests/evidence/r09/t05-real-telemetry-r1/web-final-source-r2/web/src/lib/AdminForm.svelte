<script lang="ts">
 ...
 let contentConfirmed=$state(false),contentStored=$state(false);let contentKey='';
 ...
 function clear(){contentConfirmed=false;contentStored=false;contentKey='';stopUpdates?.();stopUpdates=null;channel='UNKNOWN';generation++;who=null;base=null;latest=null;draft=formOf(null);intent=null;candidate=null;ready=false;created=null;outcomeMessage='';errors={};dialog?.close();}
 function blockConflict(){conflicted=true;latest=null;candidate=null;dialog?.close();if(!unresolved)message='Cùng phiên bản nhưng nội dung khác nhau. Chưa xác nhận dữ liệu; hãy tải và đối chiếu trước khi lưu.';}
 ...
 async function refresh(replace=false,background=false){const g=generation;await currentIdentity();const next=await snapshot();await currentIdentity();if(!mounted||g!==generation||background&&busy)return false;return observe(next,replace,background);}
 function observe(next:AdminSnapshot|null,replace=false,background=false){
  ...
  // A confirmation belongs to the observed source, even with no dirty fields.
  if(base&&next&&!sameObservation(base,next)&&candidate){candidate=null;dialog?.close();message='Dữ liệu thay đổi trong lúc xác nhận. Hãy xem lại trước khi gửi.';}
  if(base&&next&&!sameObservation(base,next)&&dirty&&!replace){latest=next;candidate=null;dialog?.close();if(!unresolved)message='Dữ liệu đã thay đổi. Phần bạn nhập vẫn được giữ; xem bản mới trước khi lưu.';return false;}
 ...
 async function restoreContent(){if(!who||!initial)return;const g=generation;const raw=sessionStorage.getItem(contentKey);if(!raw)return;const saved=decodeAdminIntent(raw,who.actor,who.epoch);if(!saved||saved.command.action!=='EDIT'||saved.command.workshopId!==initial.id||saved.state!=='FINAL')return;
  try{const result=decodeAdminResult(await json('/admin/intents/'+encodeURIComponent(saved.command.intentId)),saved);await currentIdentity();if(!mounted||g!==generation||!who)return;if(result&&(result.effect==='APPLIED'||result.effect==='NO_CHANGE')){contentConfirmed=true;contentStored=true;}}catch{if(mounted&&g===generation){contentConfirmed=false;contentStored=false;}}
 }
 async function accept(raw:unknown,pending:AdminIntent){const result=decodeAdminResult(raw,pending);if(!result)throw new Error('UNCONFIRMED');const g=generation;pending.state='FINAL';intent=pending;persist(JSON.stringify(pending));if(initial&&pending.command.action==='EDIT'&&(result.effect==='APPLIED'||result.effect==='NO_CHANGE')){contentConfirmed=true;try{sessionStorage.setItem(contentKey,JSON.stringify(pending));contentStored=true;}catch{contentStored=false;}}outcomeMessage=adminResultText(result);message='';errors=result.code==='BELOW_ACTIVE'?{capacity:outcomeMessage}:{};if(pending.command.action==='CREATE'&&result.code==='CREATED')created=result.workshopId??null;
 ...
 async function confirm(){if(!candidate||busy||!base||!who)return;busy=true;const g=generation;const command=candidate;const before=base;candidate=null;dialog.close();
  try{await currentIdentity();const current=await snapshot();await currentIdentity();if(!mounted||g!==generation||!who)return;if(current&&compareAdminObservation(before,current)==='OLDER'){message='Bản đọc cũ hơn dữ liệu đã biết. Hãy tải và đối chiếu lại trước khi gửi.';return;}if(current&&compareAdminObservation(before,current)==='CONFLICT'){blockConflict();return;}if(!current||!sameObservation(before,current)){latest=current;message='Dữ liệu thay đổi trong lúc xác nhận. Hãy xem lại trước khi gửi.';return;}await send(command);}catch{if(mounted&&g===generation)message='Chưa xác nhận dữ liệu mới. Chưa gửi thao tác.';}finally{busy=false;}
 }
 onMount(()=>{mounted=true;const g=++generation;
  void(async()=>{try{const session=await identity();if(!mounted||g!==generation)return;who=session;contentKey='workshop-admin-content:'+session.actor+':'+session.epoch+':'+(initial?.id??'CREATE');key='workshop-admin:'+session.actor+':'+session.epoch+':'+(initial?.id??'CREATE');base=await snapshot();await currentIdentity();if(!mounted||g!==generation)return;draft=formOf(base);const saved=sessionStorage.getItem(key);if(saved){const restored=decodeAdminIntent(saved,session.actor,session.epoch);if(!restored){message='Mã đối chiếu đã lưu không hợp lệ. Chưa thể tạo thao tác thay thế.';return;}intent=restored;}await restoreContent();if(!mounted||g!==generation||!who)return;ready=true;if(initial){stopUpdates=openUpdates({audience:'admin',epoch:session.epoch,workshopId:initial.id,csrf:()=>who?.csrf??'',validate:async()=>{const before=generation;await currentIdentity();return mounted&&before===generation;},read:()=>json('/admin/workshops/'+encodeURIComponent(initial.id)),snapshot:(raw,source)=>{const next=decodeAdmin(raw);if(!next||next.id!==initial.id||next.epoch!==who?.epoch||busy)return false;return observe(next,false,source==='SOCKET');},status:value=>{if(mounted)channel=value;}});}message=unresolved?'Có thao tác chưa xác nhận. Chỉ đọc kết quả cùng mã; không gửi lại.':'Có thể chỉnh sửa. Trạng thái được kiểm lại trước khi gửi.';}catch{if(mounted&&g===generation){clear();message='Chưa xác nhận được phiên hoặc dữ liệu quản trị.';}}})();
...
{#if ready&&who}
 {#if contentConfirmed}<section aria-label="Lần lưu nội dung đã xác nhận"><p>Lần lưu nội dung trước đã được máy chủ xác nhận. Đây là kết quả của lần đó, không xác nhận việc xuất bản hay trạng thái hiện tại.</p>{#if !contentStored}<p>Dấu đối chiếu lịch sử chỉ được giữ trong trang hiện tại; chưa lưu được để khôi phục sau khi tải lại.</p>{/if}</section>{/if}
