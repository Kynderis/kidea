...
Independent cases in the original admin functional group pause 1100ms after their assertions finish, with the requested/actual cadence saved. This prevents an automated burst across cases from consuming the unchanged admission budget. There is no delay inside a race, double click, confirmation or response-loss stimulus, no hidden HTTP429 retry, and every original assertion/expected200 remains. Page API request/response timestamps/statuses and explicit admin reads are recorded for diagnosis. This functional pacing is not WQ/load or response-time proof; any unexpected429 still fails the run.
