...
def audit_observation(db, out, checks):
    ...
    # Bound this phase by durable row IDs, independently of clocks. The new
    # operations phase has its own exhaustive audit; no later intent is silently
    # attributed to observation or omitted from the combined audit.
    ...
    next_proof = out / 'browser/operations-evidence.json'
    end = db.execute('SELECT coalesce(max(rowid),0)+1 FROM admin_audit').fetchone()[0]
    if next_proof.exists():
        following = json.loads(next_proof.read_text())['created']
        assert following['command']['intentId'] == 'ops-real-source-create'
        next_rows = db.execute('SELECT rowid FROM admin_audit WHERE intent_id=?',(following['command']['intentId'],)).fetchall()
        assert len(next_rows)==1
        end = next_rows[0][0]
        assert end > boundary
    for row in db.execute("SELECT rowid,intent_id FROM admin_audit WHERE action!='DENIED'"):
        assert (boundary<=row[0]<end) == (row[1] in owned), 'owned audit phase boundary'
    rows = db.execute("SELECT * FROM admin_audit WHERE action='DENIED' AND rowid>? AND rowid<?",(boundary,end)).fetchall()
