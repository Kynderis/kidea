...
const xml='<testsuite tests="'+expected.cases.length+'" failures="1" disabled="0" skipped="0">'+expected.cases.map(id=>`<testcase name="${id}" status="${id==='C01'?'fail':'run'}">${id==='C01'?'<failure message="Failed"/>':''}<system-out/></testcase>`).join('')+'</testsuite>';
