...
export function checkJUnit(xml,results,ctestExit){
  ...
  for(const [key,value]of Object.entries({tests:expected.cases.length,failures,disabled:0,skipped:0}))if(header.match(new RegExp('\\b'+key+'="(\\d+)"'))?.[1]!==String(value))throw Error('JUNIT_TOTAL');
}
