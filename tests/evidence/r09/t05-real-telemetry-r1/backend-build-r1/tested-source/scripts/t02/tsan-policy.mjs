...
export function classify({stderr,code,signal=null,timedOut=false,oom=false,context}) {
  ...
  for (const b of reports) {
    ...
    const accesses=[...b.matchAll(/^  (Write|Read|Previous read|Previous write) of size 8 at (0x[0-9a-f]+) by thread T\d+ \(mutexes: write M\d+\):\n((?:    #[^\n]+\n)+)/gm)];
    if (accesses.length!==2 || accesses[0][2]!==accesses[1][2]) return blocked('ACCESS_PAIR');
    // TSan prints the current access before the prior access. Scheduling can
    // reverse the display order without changing the approved writer/reader.
    const writeFirst=accesses[0][1]==='Write'&&accesses[1][1]==='Previous read';
    const readFirst=accesses[0][1]==='Read'&&accesses[1][1]==='Previous write';
    if(!writeFirst&&!readFirst)return blocked('ACCESS_PAIR');
    const writeAccess=accesses[writeFirst?0:1],readAccess=accesses[writeFirst?1:0];
    ...
    const writer=frame(writeAccess[3],'walIndexWriteHdr','memcpy');
    const headerReader=frame(readAccess[3],'walIndexTryHdr','memcpy');
    const validationReader=frame(readAccess[3],'walTryBeginRead','memcmp');
