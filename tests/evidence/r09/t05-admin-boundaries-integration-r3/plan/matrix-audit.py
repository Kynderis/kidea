...
try:
    ...
    assert {(r['namespace'], r['owner'], r['actor']) for r in rows} == {('registration', 'U', 'U'), ('registration', 'V', 'V'), ('admin', '', 'B')}
    expected = {('registration', 'U'): ns['ur'], ('registration', 'V'): ns['vr'], ('admin', ''): ns['br']}
    assert all(json.loads(r['result']) == expected[(r['namespace'], r['owner'])] for r in rows)
    for actor, reply in [('U', ns['ur']), ('V', ns['vr'])]:
