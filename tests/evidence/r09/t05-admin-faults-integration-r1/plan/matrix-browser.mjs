...
export async function checkMatrix({context,base,record,closeExisting}){
 ...
 try{
  ...
  for(let n=0;n<2;n++){
   ...
   await mp.getByRole('button',{name:'Kiểm tra kết quả',exact:true}).click();const absent=await lookup;assert.equal(absent.status(),202);const unknown=await absent.json();assert.equal(unknown.finality,'UNKNOWN');
   ...
   assert.equal(dropCount,1);assert.deepEqual(await read(missingId),missingBefore);
