...
const effects:Record<string,string>={REGISTERED:'APPLIED',ALREADY_REGISTERED:'NO_CHANGE',FULL:'REJECTED',LIMIT_REACHED:'REJECTED',PAUSED:'REJECTED',CANCELLED:'APPLIED',ALREADY_CANCELLED:'NO_CHANGE'};
...
export function decodeReply(raw:unknown,intent:Intent):FinalReply|null {
 ...
 if(cancel&&['REGISTERED','ALREADY_REGISTERED','FULL','LIMIT_REACHED'].includes(v.code))return null;
 if(!cancel&&['CANCELLED','ALREADY_CANCELLED'].includes(v.code))return null;
 if(!['PAUSED','FULL','LIMIT_REACHED'].includes(v.code)&&(typeof v.registrationId!=='string'||!v.registrationId))return null;
