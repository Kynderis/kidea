...
const check=(id,details={})=>{results.push({...details,id,status:'PASS'});fs.writeFileSync('/out/progress.json',JSON.stringify(results));};
...
}catch(e){fs.writeFileSync('/out/result.json',JSON.stringify({status:'FAIL',error:e.stack,checks:results,timings,wire,consoleErrors},null,2));if(page)try{await page.screenshot({path:'/out/failure.png',fullPage:true});fs.writeFileSync('/out/failure.html',await page.content());}catch{/* The page may have closed during the owned fault; keep the recorded FAIL. */}process.exitCode=1;}
