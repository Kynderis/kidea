...
try{
 ...
 for(const kind of ['older-read','actor-during-read','epoch-during-read','snapshot-503','snapshot-malformed','session-401','session-403','session-503'])await test('confirmation '+kind+' never sends an unconfirmed or wrong-session POST',async()=>{
  ...
  try{
   ...
   await p.getByRole('button',{name:'Xác nhận thay đổi',exact:true}).click();await p.waitForFunction(()=>!document.querySelector('dialog[open]')&&!document.querySelector('fieldset[disabled]')&&/Dữ liệu thay đổi|Chưa xác nhận dữ liệu mới|Chưa xác nhận được quyền|Phiên đã thay đổi|Bản đọc/.test(document.querySelector('p[role=status]')?.textContent??''));
