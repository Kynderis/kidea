 ...
 try{
  ...
  for(const action of ['CREATE','EDIT','STATE']){
   ...
   try{
    ...
    const beforeScroll=await button.boundingBox();await button.scrollIntoViewIfNeeded();const box=await button.boundingBox();assert.ok(box);const viewport=dp.viewportSize();assert.ok(box.x>=0&&box.y>=0&&box.x+box.width<=viewport.width&&box.y+box.height<=viewport.height);
    evidence.pointerTargets??=[];evidence.pointerTargets.push({action,beforeScroll,box,viewport});
    // Attach both outcomes immediately and retain the original response deadline.
    // A missing first POST must fail through our diagnostics, not an unhandled
    // rejection while an unbounded arrival promise is still pending.
    const response=postResponse(dp).then(value=>({value}),error=>({error}));
    await dp.mouse.click(box.x+box.width/2,box.y+box.height/2);const arrival=await Promise.race([arrivedReply.then(()=>null),response]);if(arrival!==null)throw arrival.error??new Error('POST_RESPONSE_BEFORE_HELD_REQUEST');
    await dp.mouse.click(box.x+box.width/2,box.y+box.height/2);assert.equal(await dp.locator('fieldset[disabled]').count(),1);
    assert.equal(captured.length,1);releaseReply();const received=await response;if(received.error)throw received.error;const r=received.value;assert.equal(r.status(),200);const result=await r.json();assert.equal(result.code,action==='CREATE'?'CREATED':'UPDATED');
