...
std::optional<Session> session(const std::string &token) {
  ...
  if (it == sessions.end() || it->second.revoked || it->second.expires <= Clock::now())
    return std::nullopt;
  return it->second;
}
...
drogon::HttpResponsePtr response(drogon::HttpStatusCode code, const Json::Value &value) {
...
bool csrf(const drogon::HttpRequestPtr &req, const Session &s) {
  return req->getHeader("origin") == origin &&
         equal_secret(req->getHeader("x-csrf-token"), s.csrf) && req->getParameter("token").empty();
}
...
public:
  void handleNewMessage(const drogon::WebSocketConnectionPtr &connection, std::string &&,
  ...
  void handleNewConnection(const drogon::HttpRequestPtr &req,
    ...
    connection->setContext(std::make_shared<std::string>(req->getCookie("__Host-kidea_session")));
...
int main() {
  try {
    // Synthetic fault injection for this lab only; never supplied by HTTP input.
    int lab_write_delay_ms = 0;
    if (const char *delay = std::getenv("KIDEA_LAB_WRITE_DELAY_MS")) {
      std::size_t consumed = 0;
      lab_write_delay_ms = std::stoi(delay, &consumed);
      if (consumed != std::string(delay).size() || lab_write_delay_ms < 0 ||
          lab_write_delay_ms > 60000)
        throw std::runtime_error("Invalid lab write delay");
    }
    ...
    for (const auto &item : (*fixture)["sessions"]) {
      sessions.emplace(
          item["token"].asString(),
          Session{item["actor"].asString(), item["csrf"].asString(), item["role"].asString(),
                  Clock::now() + std::chrono::seconds(item.get("ttl", 900).asInt()), false});
    }
    kidea::Store store("/data/sample.db");
    kidea::Worker writer;
    auto &app = drogon::app();
    app.registerFilter(std::make_shared<SocketAuth>());
    if (!std::dynamic_pointer_cast<SocketAuth>(drogon::DrClassMap::getSingleInstance("SocketAuth")))
    ...
    // Drogon's default handler logs/allocates inside the signal context.
    // Only publish a lock-free flag here; the event-loop timer drains normally.
    app.disableSigtermHandling();
    struct sigaction termination {};
    termination.sa_handler = [](int) { stopping.store(true, std::memory_order_relaxed); };
    sigemptyset(&termination.sa_mask);
    if (sigaction(SIGTERM, &termination, nullptr) != 0 ||
        sigaction(SIGINT, &termination, nullptr) != 0)
      throw std::runtime_error("Cannot install termination handler");
    app.registerPreSendingAdvice(
        [](const drogon::HttpRequestPtr &req, const drogon::HttpResponsePtr &res) {
          if (req->path() == "/api/ws" && res->statusCode() == drogon::k101SwitchingProtocols)
    ...
    app.registerHandlerViaRegex(
        ...
        [&](const drogon::HttpRequestPtr &req, Callback &&callback) {
          ...
          if (req->path() == "/api/refresh" && req->method() == drogon::Post) {
            ...
            out->addHeader("Set-Cookie", "__Host-kidea_session=" + token +
          ...
          if (req->path() == "/api/logout" && req->method() == drogon::Post) {
            ...
            out->addHeader("Set-Cookie", "__Host-kidea_session=; Path=/; Secure; HttpOnly; "
          ...
          if (req->path() == "/api/result" && req->method() == drogon::Get) {
            ...
            if (!writer.submit([&, callback, actor = s->actor, id] {
                  callback(response(drogon::k200OK, message(store.lookup(actor, id))));
                }))
              fail(callback, drogon::k503ServiceUnavailable);
            return;
          }
          ...
          if (req->path() == "/api/write" && req->method() == drogon::Post) {
            ...
            if (!body || !(*body)["id"].isString() || !(*body)["value"].isString() ||
            ...
            const auto id = (*body)["id"].asString(), value = trim((*body)["value"].asString());
            if (!writer.submit([&, callback, id, value, actor = s->actor] {
                  if (lab_write_delay_ms != 0) {
                    std::cerr << "LAB_WRITE_ENTERED\n";
                    std::this_thread::sleep_for(std::chrono::milliseconds(lab_write_delay_ms));
                  }
                  const auto result = store.apply(actor, id, value);
                  callback(response(result == "SUCCESS" ? drogon::k200OK : drogon::k409Conflict,
                                    message(result)));
                }))
              fail(callback, drogon::k503ServiceUnavailable);
            return;
    ...
    app.getLoop()->runEvery(0.02, [&] {
      ...
      if (writer.pending() == 0 && delayed == 0)
        app.quit();
      else if (Clock::now() - *stopped >= std::chrono::seconds(30)) {
        // A blocked worker cannot be safely cancelled/joined. Terminate the
        // process at the deadline; SQLite WAL recovery owns any unacknowledged
        // transaction. Already acknowledged commits remain durable. Do not run
        // destructors that would wait indefinitely for the blocked writer.
        std::cerr << "Drain deadline reached; unfinished outcomes UNKNOWN\n";
