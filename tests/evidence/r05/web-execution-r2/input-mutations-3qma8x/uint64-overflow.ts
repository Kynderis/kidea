...
export function validVersion(v: unknown): v is string {
 return typeof v === 'string' && /^(0|[1-9][0-9]{0,19})$/.test(v);
}
...
export function decodeReply(raw: string, context: Context, intentId: string): 'UNKNOWN' | 'SUCCESS' | 'REJECTED' {
 const v=parse(raw);
 if(!record(v) || v.actor!==context.actor || v.epoch!==context.epoch || v.generation!==context.generation || v.intentId!==intentId || v.namespace!=='admin' || v.state!=='FINAL')return 'UNKNOWN';
