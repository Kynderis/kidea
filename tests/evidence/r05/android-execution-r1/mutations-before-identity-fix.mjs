...
const cases=[
 ...
 ['actor-cache','"${identity.actor}:${identity.epoch}:${identity.workshop}"','"${identity.epoch}:${identity.workshop}"','cacheSeparatesActorEpochAndWorkshop'],
...
const revision=process.argv[2]??'';if(!['','sdk37'].includes(revision))throw Error('Mutation revision');
