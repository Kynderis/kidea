...
bool plain_text(std::string_view value, std::size_t maximum, bool title) {
  ...
  while (cmark_iter_next(iter.get()) != CMARK_EVENT_DONE) {
    ...
    if (type != CMARK_NODE_DOCUMENT && type != CMARK_NODE_PARAGRAPH &&
        type != CMARK_NODE_TEXT && type != CMARK_NODE_SOFTBREAK)
      return false;
  }
  return true;
}
...
public:
  ...
  void bind(int index, const std::string &data) {
    if (data.size() >
        ...
        sqlite3_bind_text(value, index, data.data(),
  ...
  try {
    ...
    if (sqlite3_step(mode.value) != SQLITE_ROW ||
        std::string(reinterpret_cast<const char *>(
            sqlite3_column_text(mode.value, 0))) != "wal")
...
std::string Store::apply(const std::string &actor, const std::string &id,
  ...
  try {
    ...
    if (row == SQLITE_ROW) {
      const auto text = [&](int column) {
        return std::string(reinterpret_cast<const char *>(
            sqlite3_column_text(prior.value, column)));
      };
      const std::string result =
          (text(0) == actor && text(1) == payload) ? text(2) : "CONFLICT";
      exec("ROLLBACK");
      transaction = false;
      return result;
    }
    ...
    const std::array<const char *, 4> sql = {
        ...
        "INSERT INTO results VALUES(?,?,?,'SUCCESS')",
        "INSERT INTO audit VALUES(?,?)", "INSERT INTO outbox VALUES(?,?)"};
    const std::array<std::string_view, 4> stages = {"domain", "result", "audit",
                                                    "outbox"};
...
Counts Store::counts() {
  ...
  const std::array<const char *, 4> sql = {
      "SELECT count(*) FROM domain", "SELECT count(*) FROM results",
      "SELECT count(*) FROM audit", "SELECT count(*) FROM outbox"};
  for (std::size_t i = 0; i < sql.size(); ++i) {
