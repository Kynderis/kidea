...
cmark_parser *cmark_parser_new_with_mem_into_root(int options, cmark_mem *mem, cmark_node *root) {
  ...
  parser->kidea_has_reference_definition = false;
  parser->root = root;
  parser->current = root;
...
int cmark_parser_has_reference_definition(cmark_parser *parser) {
  return parser->kidea_has_reference_definition ? 1 : 0;
}
...
static bool resolve_reference_link_definitions(cmark_parser *parser) {
  ...
  while (chunk.len && chunk.data[0] == '[' &&
         (pos = cmark_parse_reference_inline(parser->mem, &chunk,
                                             parser->refmap))) {
    parser->kidea_has_reference_definition = true;
