...
std::string db_file() {
  // Docker restart can reuse PIDs while retaining /tmp. CREATE-exclusive names
  // keep crash/busy tests independent of every earlier run's database state.
  char name[] = "/tmp/kidea-db-XXXXXX";
  const int fd = mkstemp(name);
  require(fd >= 0, "create isolated database fixture");
  require(close(fd) == 0, "close isolated database fixture");
  return name;
}
void grammar() {
