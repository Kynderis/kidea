...
await check('TLS SNI and forged Host mismatch rejected',async()=>assert.equal((await request('/api/health',{headers:{host:'evil.test:8443'}})).status,421));
await check('wrong authority port rejected',async()=>assert.equal((await request('/api/health',{headers:{host:'localhost:9443'}})).status,400));
...
await check('invalid input rejected before any queued write',async()=>{
 const invalid=[
  '{"id":"invalid","value":"a","value":"b","version":"1"}',
  Buffer.from([0xff,0xfe]),
  JSON.stringify({id:'invalid',value:'a',version:'18446744073709551616'}),
  JSON.stringify({id:'invalid',value:'a'.repeat(121),version:'1'}),
  JSON.stringify({id:'invalid',value:'**markup**',version:'1'}),
  JSON.stringify({id:'invalid',value:' \t\r\n',version:'1'})
 ];
 for(const body of invalid)assert.equal((await request('/api/write',mutate(a,{body}))).status,400);
...
await check('Unicode and uint64 string boundary accepted over HTTP',async()=>{
 const initial=await counts();
 for(const value of ['😀','é','a'.repeat(120)])assert.equal((await request('/api/write',mutate(a,{body:JSON.stringify({id:randomUUID(),value,version:'18446744073709551615'})}))).status,200);
 const after=await counts();for(const k of ['domain','result','audit','outbox'])assert.equal(after[k],initial[k]+3);
});
await check('concurrent duplicate intent has one authoritative four-table result',async()=>{
 const initial=await counts(),id=randomUUID();
 const body=JSON.stringify({id,value:'same intent',version:'1'});
 const replies=await Promise.all([request('/api/write',mutate(a,{body})),request('/api/write',mutate(a,{body}))]);
 for(const reply of replies)assert.equal(reply.status,200);
 const conflicting=await request('/api/write',mutate(a,{body:JSON.stringify({id,value:'different payload',version:'1'})}));
 assert.equal(conflicting.status,409);
 const after=await counts();for(const k of ['domain','result','audit','outbox'])assert.equal(after[k],initial[k]+1);
});
