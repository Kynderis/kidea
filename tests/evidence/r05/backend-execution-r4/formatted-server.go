...
func (s *Server) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	// Kidea lab: Go's transport read limit includes a 4096-byte buffer allowance.
	// Enforce the configured decoded header budget before any route or upstream.
	// This counts canonical header fields (including Host), not compressed H2 bytes
	// or whitespace discarded by the HTTP parser; transport limits still bound those.
	if s.MaxHeaderBytes > 0 && kideaHeaderBytes(r) > int64(s.MaxHeaderBytes) {
