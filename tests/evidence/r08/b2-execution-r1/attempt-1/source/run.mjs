...
const started=Date.now(),free=()=>{const s=fs.statfsSync(out);return s.bavail*s.bsize;},initialFree=free();
...
async function start(component,command,extra=[],image=input.toolchain,network=context.network){
 ...
 await docker(deployment({target:activeTarget,component,name,network,image,mounts:mounts(component),command,env:extra}));return name;
}
...
try {
 ...
 for(const target of ['lab-dev','lab-prod']){
  ...
  for(const stuck of [false,true]){
   ...
   const began=Date.now();await docker(['exec',caddy,'node','/plan/identity.mjs','caddy',input.caddy.sha256,'term']);assert.equal(Number(await docker(['wait',caddy])),0);const elapsed=Date.now()-began;
