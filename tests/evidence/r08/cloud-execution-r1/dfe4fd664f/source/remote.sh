...
load)
 ...
 # Classic Docker identifies this exact OCI export by its config digest.
 image=$(cat /var/lib/kidea-r08/payload/image-id)
 actual=$(sudo docker image inspect "$image" --format '{{.Id}}')
 test "$actual" = "$image"
 sudo docker image inspect "$image" --format '{{json .RootFS.Layers}}'
 # COS data mounts are noexec. Docker manages the executable payload volume.
 sudo docker volume create kidea-r08-payload
 sudo docker run --rm --pull=never --network=none --read-only --user=0:0 --cap-drop=ALL --security-opt=no-new-privileges --cpus=1 --memory=512m --pids-limit=128 -v /var/lib/kidea-r08/payload:/input:ro -v kidea-r08-payload:/payload "$image" timeout 120 sh -c 'cp -R /input/. /payload/; chmod -R a+rX /payload; diff -qr /input /payload'
 ;;
workload)
 ...
 if test -f /home/kidealab/restore.db; then
  ...
  sudo docker run --rm --pull=never --network=none --read-only --user=1000:1000 --cap-drop=ALL --security-opt=no-new-privileges --cpus=0.5 --memory=512m -v kidea-r08-payload:/payload:ro -v "$PWD/data:/data" "$image" /payload/database backup /data/input.db /data/sample.db
 fi
 sudo docker run -d --name kidea-r08-backend --pull=never --network=kidea-r08-cloud --network-alias=backend --restart=unless-stopped --read-only --user=1000:1000 --cap-drop=ALL --security-opt=no-new-privileges --cpus=0.5 --memory=768m --memory-swap=768m --pids-limit=128 --tmpfs=/tmp:rw,size=128m --log-opt=max-size=4m --log-opt=max-file=2 -v kidea-r08-payload:/payload:ro -v "$PWD/payload/config:/secrets:ro" -v "$PWD/data:/data" "$image" /payload/backend
 sudo docker run -d --name kidea-r08-web --pull=never --network=kidea-r08-cloud --network-alias=web --restart=unless-stopped --read-only --user=1000:1000 --cap-drop=ALL --security-opt=no-new-privileges --cpus=0.5 --memory=768m --memory-swap=768m --pids-limit=128 --tmpfs=/tmp:rw,size=128m --log-opt=max-size=4m --log-opt=max-file=2 -v kidea-r08-payload:/payload:ro --workdir=/payload/web -e KIDEA_INTEGRATION=1 -e HOST=0.0.0.0 -e PORT=4173 -e ORIGIN=https://localhost:8443 -e BODY_SIZE_LIMIT=131072 -e SHUTDOWN_TIMEOUT=30 "$image" node /payload/web/build/index.js
 sudo docker run -d --name kidea-r08-caddy --pull=never --network=kidea-r08-cloud --network-alias=caddy --restart=unless-stopped --read-only --user=1000:1000 --cap-drop=ALL --security-opt=no-new-privileges --cpus=0.25 --memory=512m --memory-swap=512m --pids-limit=128 --tmpfs=/tmp:rw,size=128m --log-opt=max-size=4m --log-opt=max-file=2 -p 8443:8443 -v kidea-r08-payload:/payload:ro -v "$PWD/payload/ca:/data" -e XDG_DATA_HOME=/data -e XDG_CONFIG_HOME=/tmp/config "$image" /payload/caddy run --config /payload/Caddyfile --adapter caddyfile
 sudo docker run -d --name kidea-r08-backup --pull=never --network=kidea-r08-cloud --restart=unless-stopped --read-only --user=1000:1000 --cap-drop=ALL --security-opt=no-new-privileges --cpus=0.5 --memory=512m --memory-swap=512m --pids-limit=128 --tmpfs=/tmp:rw,size=64m --log-opt=max-size=4m --log-opt=max-file=2 -p 9444:9444 -v kidea-r08-payload:/payload:ro -v "$PWD/payload/config:/config:ro" -v "$PWD/data:/data:ro" -v "$PWD/backup:/backup" "$image" node /payload/backup-server.mjs
 ;;
observer)
 cd /var/lib/kidea-r08
 image=$(cat payload/image-id)
 sudo docker run -d --name kidea-r08-observer --pull=never --network=host --restart=unless-stopped --read-only --user=1000:1000 --cap-drop=ALL --security-opt=no-new-privileges --cpus=0.5 --memory=512m --memory-swap=512m --pids-limit=128 --tmpfs=/tmp:rw,size=64m --log-opt=max-size=4m --log-opt=max-file=2 -v kidea-r08-payload:/payload:ro -v "$PWD/payload/config:/config:ro" -v "$PWD/state:/state" "$image" node /payload/observer.mjs
 ;;
snapshot)
