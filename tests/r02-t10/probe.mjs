// D2-only controller. D1 evidence is read-only; no retries or replacement roots.
...
export const runRoot=path.join(repo,'.test-output/r02-t10/probe-r2');
const baselineRoot=path.join(repo,'.test-output/r02-t10/probe-r1');
const baselineManifestHash='c812d4fdb8cc06c2f6da2dc93e544a8f42c2169322f83d29bf1035e9b71d1fae';
const baselineSummaryHash='0b086738c801241c582cf776675cad9f3bc14a70c85a411c0bd43c5274e0f82a';
...
const proposal='proposals/r02-t10-probe-r2.md';
...
function sourceFiles() {
  const list=[proposal,'KIDEA_DESIGN.md','KIDEA_QUALITY.md','KIDEA_ACCEPTANCE.md','package.json','package-lock.json',...suites,'tests/fixtures/r02-t04/catalog.mjs','.test-output/r02-t10/probe-r1/manifest.json','.test-output/r02-t10/probe-r1/summary.json'];
...
// Compare regenerated bytes/expected values against the retained D1 manifest,
// never against status output or merely the minimum workload size.
export function assertBaselineFixture(built,baseline) {
  assert.ok(baseline,'Missing D1 workload');
  assert.deepEqual(built.stats,baseline.stats,'D1 workload stats changed');
  assert.deepEqual(Object.fromEntries([...built.files].map(([p,b])=>[p,{sha256:hash(b),bytes:b.length}])),baseline.inventory.files,'D1 fixture bytes changed');
  assert.equal(hash(JSON.stringify(built.expected,null,2)+'\n'),baseline.expectedHash,'D1 expected output changed');
}
function baselineState() {
  checkPath(baselineRoot);
  const manifest=readFileSync(path.join(baselineRoot,'manifest.json')),summary=readFileSync(path.join(baselineRoot,'summary.json'));
  assert.equal(hash(manifest),baselineManifestHash,'D1 manifest changed');assert.equal(hash(summary),baselineSummaryHash,'D1 summary changed');
  return {root:baselineRoot,manifestHash:baselineManifestHash,summaryHash:baselineSummaryHash,evidence:tree(baselineRoot)};
}
...
function prepare() {
  ...
  const baseline=baselineState(),d1=JSON.parse(readFileSync(path.join(baselineRoot,'manifest.json'),'utf8'));
  const environment=host(),sources=sourceFiles();
  assert.deepEqual(environment,d1.environment,'D1 host identity differs');
  assert.deepEqual({version:process.version,path:process.execPath,sha256:nodeHash},d1.node,'D1 Node identity differs');
  assert.deepEqual(sources['tests/r02-t10/fixtures.mjs'],d1.sources['tests/r02-t10/fixtures.mjs'],'D1 generator changed');
  mkdirSync(path.dirname(runRoot),{recursive:true});mkdirSync(runRoot);
  save('preparation-start.json',{at:new Date().toISOString(),authorization:'Human Ok làm đi after 3e142f7 answer, D2 only',environment,sources,baseline});
  ...
  for(const w of workloads) {
    const built=buildFixture(w),original=d1.fixtures.find(f=>f.id===w.id),root=path.join(runRoot,w.id);
    assertBaselineFixture(built,original);assert.deepEqual(tree(original.root),original.inventory,'Retained D1 fixture changed');
    mkdirSync(root);
    ...
    assert.deepEqual(inventory,original.inventory,'Regenerated D2 tree differs from D1');
  ...
  assert.ok(same(sources,sourceFiles()),'Source changed during preparation');assert.ok(same(environment,host()),'Host identity changed');assert.deepEqual(baselineState(),baseline,'D1 evidence changed during preparation');
  save('manifest.json',{schemaVersion:1,at:new Date().toISOString(),authorization:'D2 approved by Human after 3e142f7; no AI/sandbox/ACL/install',sourceBase:'3e142f7548bbb9f0bab08975f82e11a13e6a7f87',baseline,node:{version:process.version,path:process.execPath,sha256:nodeHash},sources,environment,fixtures,iterationsPerFixture:11,totalIterations:22,timeoutMs:30000,retries:0,threshold:null,
...
async function run(expectedManifestHash) {
  ...
  assert.deepEqual(baselineState(),m.baseline,'D1 evidence changed before D2 run');
  ...
  try{baselineUnchanged=same(baselineState(),m.baseline);if(!baselineUnchanged)stopReason??='D1 evidence changed';}catch(e){stopReason??=e.message;}
  const summary={at:new Date().toISOString(),manifestHash:hash(manifestBytes),baselineManifestHash,baselineUnchanged,requested:22,attempted:rows.length,notRun:22-rows.length,stopReason,verification:'EXPLORATORY_TIMINGS_NOT_QUALITY_ACCEPTANCE',workloads:Object.fromEntries(m.fixtures.map(f=>[f.id,summarize(rows.filter(r=>r.fixture===f.id))])),rows};
  save('summary.json',summary);console.log(JSON.stringify(summary.workloads));
  process.exitCode=rows.length===22&&rows.every(r=>r.success)&&baselineUnchanged?0:1;
}
