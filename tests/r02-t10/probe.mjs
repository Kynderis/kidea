// D4-only controller. D1/D2/D3 evidence is read-only; no replacement roots.
...
export const runRoot=path.join(repo,'.test-output/r02-t10/probe-r3');
const baselineRoot=path.join(repo,'.test-output/r02-t10/probe-r2');
const baselineManifestHash='bbe5bae6df558b4f75ea29a1a254182815e2c3d01cc21a000f229a803629f40d';
const baselineSummaryHash='f9fde4d6089e7cc8fd51a7f20924054d9e4f5925cc7473dd29a6e7b3af422986';
const retainedRuns=[
  ...
  ['probe-r2',baselineManifestHash,baselineSummaryHash],
...
const proposal='proposals/r02-t10-probe-r3.md';
...
// Compare regenerated bytes/expected values against the retained D2 manifest,
// never against status output or merely the minimum workload size.
export function assertBaselineFixture(built,baseline) {
  assert.ok(baseline,'Missing D2 workload');
  assert.deepEqual(built.stats,baseline.stats,'D2 workload stats changed');
  assert.deepEqual(Object.fromEntries([...built.files].map(([p,b])=>[p,{sha256:hash(b),bytes:b.length}])),baseline.inventory.files,'D2 fixture bytes changed');
  assert.equal(hash(JSON.stringify(built.expected,null,2)+'\n'),baseline.expectedHash,'D2 expected output changed');
}
...
function prepare() {
  ...
  const baseline=baselineState(),d2=JSON.parse(readFileSync(path.join(baselineRoot,'manifest.json'),'utf8'));
  const environment=host(),sources=sourceFiles();
  assert.deepEqual(environment,d2.environment,'D2 host identity differs');
  assert.deepEqual({version:process.version,path:process.execPath,sha256:nodeHash},d2.node,'D2 Node identity differs');
  assert.deepEqual(sources['tests/r02-t10/fixtures.mjs'],d2.sources['tests/r02-t10/fixtures.mjs'],'D2 generator changed');
  ...
  save('preparation-start.json',{at:new Date().toISOString(),authorization:'Human Ok nhé after 090290a answer, D4 only',environment,sources,baseline});
  ...
  for(const w of workloads) {
    const built=buildFixture(w),original=d2.fixtures.find(f=>f.id===w.id),root=path.join(runRoot,w.id);
    assertBaselineFixture(built,original);assert.deepEqual(tree(original.root),original.inventory,'Retained D2 fixture changed');
    mkdirSync(root);
    ...
    assert.deepEqual(inventory,original.inventory,'Regenerated D4 tree differs from D2');
...
async function run(expectedManifestHash) {
  ...
  assert.deepEqual(baselineState(),m.baseline,'Retained evidence changed before D4 run');
