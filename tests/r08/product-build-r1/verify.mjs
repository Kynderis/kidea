...
export function verify(root,files){
 const found=[];function walk(dir,prefix=''){for(const entry of fs.readdirSync(dir,{withFileTypes:true})){const name=prefix+entry.name;if(entry.isSymbolicLink())throw Error('SYMLINK');if(entry.isDirectory())walk(path.join(dir,entry.name),name+'/');else if(name!=='manifest.json')found.push(name);}}walk(root);
